...
export const users = pgTable("users", {
  ...
  sessionRevokedAt: timestamp("session_revoked_at"),
  // ── Referral program ──
...
export const subServiceTaxRules = pgTable("sub_service_tax_rules", {
  ...
  // TEXT intentionally allows onboarding a country without a code release.
  ...
  index("idx_sub_service_tax_rules_lookup").on(t.subServiceId, t.countryCode, t.isActive),
]);
...
export const appointments = pgTable("appointments", {
  ...
  // Payment rail selected at booking time. This column is maintained with the
  // payments row; raw SQL is used in payment flows until it is added to the
  ...
  serviceSubtotal: decimal("service_subtotal", { precision: 10, scale: 2 }).default("0.00"),
  serviceTaxRate: decimal("service_tax_rate", { precision: 5, scale: 2 }).default("0.00"),
  serviceTaxAmount: decimal("service_tax_amount", { precision: 10, scale: 2 }).default("0.00"),
  platformTaxableSubtotal: decimal("platform_taxable_subtotal", { precision: 10, scale: 2 }).default("0.00"),
  platformTaxRate: decimal("platform_tax_rate", { precision: 5, scale: 2 }).default("0.00"),
  platformTaxAmount: decimal("platform_tax_amount", { precision: 10, scale: 2 }).default("0.00"),
  paymentGatewayFeeAmount: decimal("payment_gateway_fee_amount", { precision: 10, scale: 2 }).default("0.00"),
  adminFeeAmount: decimal("admin_fee_amount", { precision: 10, scale: 2 }).default("0.00"),
...
export const insertSubServiceTaxRuleSchema = createInsertSchema(subServiceTaxRules).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});
...
export const userPackages = pgTable("user_packages", {
  ...
  pricingBreakdown:     jsonb("pricing_breakdown"),
  taxEngineVersion:     text("tax_engine_version"),
...
export type SubServiceTaxRule = typeof subServiceTaxRules.$inferSelect;
export type InsertSubServiceTaxRule = z.infer<typeof insertSubServiceTaxRuleSchema>;
...
export const providerScheduleTemplates = pgTable("provider_schedule_templates", {
  ...
  bufferAfterMins: integer("buffer_after_mins").default(5),
  isActive: boolean("is_active").default(true),
  createdAt: timestamp("created_at").defaultNow(),
  updatedAt: timestamp("updated_at").defaultNow(),
}, (t) => [
