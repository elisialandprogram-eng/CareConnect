...
export function registerAppointmentRoutes(app: Express): void {
  ...
  app.post("/api/appointments", authenticateToken, bookingLimiter, slotLimiter, async (req: AuthRequest, res: Response) => {
    ...
    try {
      ...
      // Offline methods are intentionally first-party: they remain pending
      // until the provider records receipt and are settled by the same
      // offline-fee path as cash.
      const FIRST_PARTY_METHODS = ["card", "wallet", "cash", "bank_transfer"];
      if (!FIRST_PARTY_METHODS.includes(selectedPaymentMethod)) {
        try {
      ...
      // Resolve promo discount only after all pre-promo pricing inputs are known.
      // The minimum-order comparison must match the quote/validation path:
      // pre-promo taxable subtotal, including practitioner, visit, platform, and
      // membership adjustments, but excluding payment-method adjustments.
      if (promoCode && typeof promoCode === "string" && promoCode.trim()) {
        try {
          ...
          if (promo && promo.isActive) {
            ...
            const promoBaseCurrency = String((promo as any).baseCurrency ?? "USD").toUpperCase();
            const bookingCurrency = countryCurrency(providerCountry as CountryCode);
            ...
            const prePromoSubtotal = await calculatePrePromoSubtotal({
              ...
              serviceCategory: (subRecord as any)?.category ?? _providerType ?? null,
              bookingCurrency,
              providerCurrency: bookingCurrency,
              rates: promoRates,
            });

            if (okWindow && okUses && okProvider && prePromoSubtotal >= minimumAmount) {
              let promoValue = Number(promo.discountValue);
              const promoType: "percent" | "fixed" = promo.discountType === "percentage" ? "percent" : "fixed";
              // Fixed discounts are normalized to bookingCurrency before the
              // Revenue Engine applies them.
              ...
              // H-10: increment only after the appointment is saved.
      ...
      // Keep the primary patient's profile in sync when a self-booking uses
      // a newly entered mobile number. Family-member bookings must not
      // overwrite the account holder's number.
      const bookingContactMobile =
        typeof contactMobile === "string" ? contactMobile.trim() : "";
      if (!validatedFamilyMemberId && bookingContactMobile) {
        try {
          if ((user.mobileNumber || "").trim() !== bookingContactMobile) {
            await storage.updateUser(userId, { mobileNumber: bookingContactMobile });
          }
        } catch (profileError) {
          // The appointment is already created; profile sync is non-fatal.
          console.warn("[booking] mobile profile sync failed:", profileError);
        }
      }
      ...
      if (revenueEngineResult) {
        // These columns are intentionally raw-SQL-only: they are created by
        // startup migration and are not included in the Drizzle insert schema.
        // The timestamp is initialized exactly once, immediately after the
        // appointment insert; the DB trigger rejects later financial changes.
        try {
          await pool.query(
            `UPDATE appointments SET
               ...
               re_applied_rules            = $8::jsonb,
               booking_currency            = $9,
               provider_currency           = $10,
               patient_currency            = $11,
               final_total_usd             = $12,
               pricing_engine_version      = $13,
               tax_engine_version          = $14,
               pricing_calculated_at       = $15
             WHERE id = $16`,
            [
              revenueEngineResult.commissionRate,
              revenueEngineResult.commissionAmount.toFixed(2),
              revenueEngineResult.providerGrossEarnings.toFixed(2),
              ...
              revenueEngineResult.pricingEngineVersion,
              revenueEngineResult.taxEngineVersion,
              revenueEngineResult.pricingCalculatedAt,
              appointment.id,
            ],
          );
        } catch (snapshotErr: any) {
          console.error("[booking] required pricing snapshot write failed:", snapshotErr?.message);
          throw snapshotErr;
        }
            ...
            for (const c of childAppointments) {
              await storage.transitionAppointment(
                ...
                { status: "cancelled" } as any,
                {
                  ...
                  actorUserId: null,
                  actorRole: null,
                  reason: multiErr?.message || "Multi-session booking rollback",
                  reasonCode: "multi_session_rollback",
                },
                { allowNoop: true },
              );
            }
      ...
      //   • paymentMethod === "wallet"        → full wallet when no amount is
      //     supplied; a supplied local amount enables wallet + card remainder
      ...
      if (walletApplied > 0) {
        try {
          ...
          if (remainderDue === 0 && !isOfflinePaymentMethod) {
            ...
            if (!confirmRes.ok || !confirmRes.appointment) {
              throw new Error(confirmRes.message || "Unable to confirm wallet-paid appointment");
            }
            appointment = confirmRes.appointment;
            walletPaid = true;
          }
        ...
        try {
          ...
          await storage.transitionAppointment(
            ...
            { status: "cancelled" } as any,
            {
              ...
              reasonCode: "stripe_checkout_failed",
            },
            { allowNoop: true },
          ).catch(() => {});
      ...
      // Cash and bank transfers are collected offline by the provider. Leave the
      // payment row as pending so the provider can use the "Mark payment received"
      // flow; the appointment also stays pending until the provider confirms it.
      // (Wallet/card flows handle completion through their own online hooks above.)
      ...
      try {
        ...
        // The confirmation page renders appointment.totalAmount in the booking currency.
        // Use that exact stored value for the patient email notification too; converting
        // the USD accounting snapshot can introduce rounding/currency mismatches.
        const _bookingDisplayTotal = Number(appointment.totalAmount ?? fee);
        const _fmtPatientNotifAmt = _bookingDisplayTotal > 0
          ? formatLocal(_bookingDisplayTotal, _bookingSrcCurrency)
          : undefined;
        ...
        const _fmtProv = (amountInBookingCurrency: number) => {
          ...
          const usd = _bookingSrcCurrency === "USD"
            ...
            : amountInBookingCurrency / _bookingRateVal;
  ...
  app.patch("/api/appointments/:id/status", authenticateToken, async (req: AuthRequest, res: Response) => {
    try {
      ...
      if (!existing) {
       ...
       // requested transition must be a legal one. Admin may force lifecycle
       // transitions, but payment invariants still apply below.
      if (!isAdminRole(req.user?.role)) {
      ...
      const actionVerb = statusToAction[status] || "confirm";
       const updateRes = await storage.transitionAppointment(
        req.params.id,
        { status: status as any },
        {
         ...
         { allowAdminOverride: isAdminRole(req.user?.role) },
      );
       if (!updateRes.ok || !updateRes.appointment) {
         return res.status(updateRes.status).json({
           message: updateRes.message || "Failed to update appointment",
           ...(updateRes.allowed ? { allowed: updateRes.allowed } : {}),
         });
      }
      ...
      if (isTerminalStatus(status)) {
        try {
          ...
          if (linked) {
             // A completed appointment is no longer an active care channel.
             // Lock immediately; cancellation/support states retain a brief
             // window for practical follow-up.
             const lockAt = status === "completed"
               ? new Date()
               : new Date(Date.now() + 2 * 60 * 60 * 1000);
             await storage.lockConversation(linked.id, lockAt);
          }
        } catch (lockErr) {
  ...
  app.post("/api/appointments/:id/action", authenticateToken, async (req: AuthRequest, res: Response) => {
    try {
      ...
      const txRes = await storage.transitionAppointment(
        existing.id,
        updates as any,
        {
      ...
      if (!txRes.ok || !txRes.appointment) {
        return res.status(txRes.status).json({
          message: txRes.message || "Failed to update appointment",
          ...(txRes.allowed ? { allowed: txRes.allowed } : {}),
        });
      }
  ...
  app.post("/api/appointments/:id/reschedule-response", authenticateToken, async (req: AuthRequest, res: Response) => {
    try {
      ...
      if (accept && proposedTime) {
        ...
        // Notify patient about refund if one was issued (appointment.cancelled email doesn't mention refund)
        if (refundedAmount > 0) {
          notify.paymentRefunded(existing.patientId, {
            formattedAmount: formatLocal(refundedAmount, "USD"),
            appointmentId: existing.id,
            method: "wallet",
          }).catch(err => console.error("[notify] reschedule-rejected refund", err));
        }
      }
