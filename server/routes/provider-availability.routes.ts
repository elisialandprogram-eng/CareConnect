...
export function registerProviderAvailabilityRoutes(app: Express): void {
  ...
  app.get("/api/providers/:id/available-slots", async (req: Request, res: Response) => {
    try {
      ...
      if (slots.length > 0) {
        ...
        const result = slots
          ...
          .map(s => {
            ...
            const bufferMinutes = effBufAfter;
      ...
      // Buffer-aware step: duration plus the effective after-buffer. The
      // before-buffer is enforced by isBookedByAppt() against the preceding
      // appointment; including it again in the cadence would skip valid
      // home-visit starts (e.g. 09:45 after a 09:00–09:30 visit + 15 min travel).
