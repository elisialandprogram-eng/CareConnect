...
export function registerProviderRoutes(app: Express): void {
  ...
  app.get("/api/providers", optionalAuth, async (req: AuthRequest, res: Response) => {
    try {
      ...
      const homeVisit = req.query.homeVisit === "true" || req.query.homeVisit === "1";
      const online = req.query.online === "true" || req.query.online === "1";
      const priceMin = Number.isFinite(Number(req.query.priceMin)) ? Math.max(0, Number(req.query.priceMin)) : undefined;
      const priceMax = Number.isFinite(Number(req.query.priceMax)) ? Math.max(0, Number(req.query.priceMax)) : undefined;
      ...
      const useSearch = !!(q || type || city || verifiedOnly || minRating > 0 || homeVisit || online || priceMin != null || priceMax != null);

      // ── Cache key (country-isolated) ────────────────────────────────────────
      const cacheKey = useSearch
         ? `search:${countryFilter}:${q}:${type}:${city}:${verifiedOnly}:${minRating}:${homeVisit}:${online}:${priceMin}:${priceMax}:${page}:${pageSize}`
      ...
      if (useSearch) {
        const { rows, total: t } = await storage.searchProviders({
           q: q || undefined, type: type || undefined, city: city || undefined,
            verifiedOnly, minRating, homeVisit, online, priceMin, priceMax, countryCode: countryFilter,
          approvedOnly: true,
  ...
  app.get("/api/provider/insights", authenticateToken, async (req: AuthRequest, res: Response) => {
    try {
      ...
      const repeatPatients = repeatRows.rows.map((r: any) => ({
      ...
      res.json({
        ...
        popularServices: svcRows.rows.map((r: any) => ({
          name: r.service_name,
  ...
  app.get("/api/provider/analytics", authenticateToken, async (req: AuthRequest, res: Response) => {
    try {
      ...
      // Service breakdown
      const serviceBreakdown = svcRows.rows.map((r: any) => ({
        name: r.service_name,
      ...
      // Monthly trend
      const monthlyTrend = monthRows.rows.map((r: any) => ({
        month: r.month,
        revenue: Number(r.revenue),
        bookings: Number(r.bookings),
      ...
      const packagePerformance = pkgRows.rows.map((r: any) => ({
        name: r.package_name,
  ...
  app.get("/api/reviews/provider/me", authenticateToken, async (req: AuthRequest, res: Response) => {
    try {
      ...
      const provider = await storage.getProviderByUserId(req.user.id);
      if (!provider) return res.status(404).json({ message: "Provider not found" });
       // Provider-facing review lists only contain publicly approved reviews.
  ...
  app.get("/api/provider/week-slots-summary", authenticateToken, async (req: AuthRequest, res: Response) => {
    try {
      ...
      const requestedWeekStart = String((req.query as { weekStart?: string }).weekStart ?? "").trim();
      if (requestedWeekStart && !/^\d{4}-\d{2}-\d{2}$/.test(requestedWeekStart)) {
        return res.status(400).json({ message: "weekStart must use YYYY-MM-DD" });
      }
      // Keep the endpoint useful for dashboard summary probes that omit the
      // query while preserving the caller-selected week when it is supplied.
  ...
  app.post("/api/availability/bulk", authenticateToken, async (req: AuthRequest, res: Response) => {
    try {
      ...
      // duration + effective after-buffer steps. This keeps legacy bulk
      // generation aligned with schedule-template and patient availability.
      ...
      const toCreate = dates.flatMap((rawDate) => {
        ...
        return slots.flatMap((s) => {
          ...
          const durationMins = Number.isFinite(s.slotDurationMins) && (s.slotDurationMins ?? 0) > 0
            ? Math.floor(s.slotDurationMins!)
            : 30;
          const bufferAfterMins = Number.isFinite(s.bufferAfterMins) && (s.bufferAfterMins ?? 0) >= 0
            ? Math.floor(s.bufferAfterMins!)
            : 5;
          const stepMins = durationMins + bufferAfterMins;
