...
export function registerPatientRoutes(app: Express): void {
  ...
  app.post("/api/packages/:id/purchase", authenticateToken, async (req: AuthRequest, res: Response) => {
    try {
      ...
      // from current tax settings in the webhook or dashboard.
      ...
      if (paymentMethod === "wallet") {
        ...
        if (balance >= priceUSD) {
          ...
          const activated = await storage.activateUserPackage(up.id);
          // Notify the user that their package is now active
          notify.packagePurchased(req.user!.id, {
            packageName: pkg.name,
            formattedAmount: formatLocal(priceWithTax, _pkgPriceCurrency),
            sessionsIncluded: typeof (pkg as any).sessionsIncluded === "number" ? (pkg as any).sessionsIncluded : undefined,
          }).catch(() => {});
          return res.status(201).json({ userPackage: activated, paymentRequired: false });
        }
