    ...
    try {
      ...
      if (typeof status === "string") {
        ...
        const txRes = await storage.transitionAppointment(
          req.params.id,
          // Keep audit-only fields (reason) out of the appointment UPDATE.
          // The transition service persists them on appointment_events.
          { status } as any,
          {
            action: verb as any,
          ...
          { allowAdminOverride: true, allowNoop: true },
        );
        if (!txRes.ok || !txRes.appointment) {
          return res.status(txRes.status).json({
            message: txRes.message || "Failed to update booking",
