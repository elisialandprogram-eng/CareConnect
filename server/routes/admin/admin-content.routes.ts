...
export function registerAdminContentRoutes(app: Express): void {
  ...
  // ── Review moderation ─────────────────────────────────────────────────────
  // Public/provider review endpoints intentionally filter to approved rows.
  // Admins can inspect the complete queue and make the publication decision.
  app.get("/api/admin/reviews", authenticateToken, requireAdmin, async (req: AuthRequest, res: Response) => {
    try {
      ...
      const result = await pool.query(
        `SELECT r.id, r.appointment_id, r.provider_id, r.patient_id, r.rating,
                r.comment, r.status, r.provider_reply, r.provider_reply_at, r.created_at,
                a.date AS appointment_date, a.start_time,
                pu.first_name AS patient_first_name, pu.last_name AS patient_last_name,
                pu.email AS patient_email,
                vu.first_name AS provider_first_name, vu.last_name AS provider_last_name,
                p.clinic_name
           FROM reviews r
  ...
  app.patch("/api/admin/reviews/:id/status", authenticateToken, requireAdmin, async (req: AuthRequest, res: Response) => {
    try {
      ...
      const existing = await pool.query(
        `SELECT r.id, r.provider_id, p.country_code::text AS country_code
      ...
      // Rating is derived only from approved reviews, so rejection immediately
      // removes a previously approved row from the aggregate as well.
      await storage.recomputeProviderRating(row.provider_id);
      return res.json(updated.rows[0]);
    } catch (error) {
      console.error("[PATCH /api/admin/reviews/:id/status]", error);
      ...
      const { rows } = await pool.query(`
        INSERT INTO sub_services (
          ...
          platform_fee, base_price, duration_minutes,
           buffer_before, buffer_after, pricing_type,
