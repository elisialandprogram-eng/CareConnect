...
export function registerRevenueBillingRoutes(app: Express): void {
  ...
  // ══════════════════════════════════════════════════════════════════════════
  // PHASE 3.1 — CANONICAL TAX RULE MANAGEMENT
  // ══════════════════════════════════════════════════════════════════════════
  app.get("/api/admin/tax-rules", ...mw, async (req: Request, res: Response) => {
    try {
      const params: unknown[] = [];
      const conditions: string[] = [];
      if (req.query.subServiceId) {
        params.push(String(req.query.subServiceId));
        conditions.push(`r.sub_service_id = $${params.length}`);
      }
      ...
      const [serviceRules, platformRules] = await Promise.all([
        pool.query(
          `SELECT r.*, s.name AS sub_service_name
           FROM sub_service_tax_rules r
           JOIN sub_services s ON s.id = r.sub_service_id
           ${where}
           ORDER BY r.country_code, s.name, r.effective_from DESC`,
          params,
        ),
        pool.query(
          `SELECT * FROM tax_settings
           ORDER BY country, effective_from DESC NULLS LAST, year DESC NULLS LAST, id DESC`,
        ),
      ]);
      res.json({
        serviceRules: serviceRules.rows.map(camelize),
        platformRules: platformRules.rows.map(camelize),
      });
    } catch (err: any) {
      res.status(500).json({ message: "Failed to load tax rules", error: err.message });
    }
  });

  app.post("/api/admin/tax-rules/service", ...mw, async (req: Request, res: Response) => {
    try {
      const {
        subServiceId,
        countryCode,
      ...
      if (!subServiceId || !countryCode) {
        return res.status(400).json({ message: "subServiceId and countryCode are required" });
      }
      const result = await pool.query(
        `INSERT INTO sub_service_tax_rules
          (sub_service_id, country_code, tax_rate, effective_from, effective_to, is_active)
      ...
      res.status(400).json({ message: "Invalid service tax rule", error: err.message });
    }
  });

  app.patch("/api/admin/tax-rules/service/:id", ...mw, async (req: Request, res: Response) => {
    try {
      ...
      for (const [key, column] of Object.entries(allowed)) {
        ...
        values.push(key === "countryCode"
          ? String(req.body[key]).toUpperCase()
          : key === "isActive" ? Boolean(req.body[key]) : req.body[key] || null);
        set.push(`${column} = $${values.length}${column.includes("effective") ? "::timestamptz" : ""}`);
      }
      if (!set.length) return res.status(400).json({ message: "No editable fields supplied" });
      values.push(req.params.id);
      const result = await pool.query(
        `UPDATE sub_service_tax_rules SET ${set.join(", ")}, updated_at = NOW()
         WHERE id = $${values.length} RETURNING *`,
        values,
      );
      if (!result.rows.length) return res.status(404).json({ message: "Service tax rule not found" });
  ...
  app.delete("/api/admin/tax-rules/service/:id", ...mw, async (req: Request, res: Response) => {
    try {
      const result = await pool.query(`DELETE FROM sub_service_tax_rules WHERE id = $1 RETURNING id`, [req.params.id]);
      if (!result.rows.length) return res.status(404).json({ message: "Service tax rule not found" });
      res.status(204).end();
    } catch (err: any) {
      res.status(500).json({ message: "Failed to delete service tax rule", error: err.message });
    }
  });

  app.post("/api/admin/tax-rules/platform", ...mw, async (req: Request, res: Response) => {
    try {
      const {
        country,
        countryCode,
      ...
      const resolvedCountry = String(countryCode ?? country ?? "").toUpperCase();
      if (!resolvedCountry) return res.status(400).json({ message: "countryCode is required" });
      const result = await pool.query(
        `INSERT INTO tax_settings
          (country, tax_name, tax_rate, is_active, year, effective_from, effective_to, updated_at)
        ...
        [
          ...
          String(taxRate),
          Boolean(isActive),
          year == null ? new Date().getFullYear() : Number(year),
          effectiveFrom || null,
          effectiveTo || null,
        ],
      );
      ...
      res.status(400).json({ message: "Invalid platform tax rule", error: err.message });
    }
  });

  app.patch("/api/admin/tax-rules/platform/:id", ...mw, async (req: Request, res: Response) => {
    try {
      const allowed: Record<string, string> = {
        country: "country",
        countryCode: "country",
      ...
      for (const [key, column] of Object.entries(allowed)) {
        if (!(key in (req.body ?? {}))) continue;
        values.push(key === "country" || key === "countryCode"
          ? String(req.body[key]).toUpperCase()
          : key === "isActive" ? Boolean(req.body[key]) : req.body[key] || null);
        set.push(`${column} = $${values.length}${column.startsWith("effective_") ? "::timestamptz" : ""}`);
      }
      if (!set.length) return res.status(400).json({ message: "No editable fields supplied" });
      values.push(req.params.id);
      const result = await pool.query(
        `UPDATE tax_settings SET ${set.join(", ")}, updated_at = NOW()
         WHERE id = $${values.length} RETURNING *`,
        values,
      );
      if (!result.rows.length) return res.status(404).json({ message: "Platform tax rule not found" });
      res.json(camelize(result.rows[0]));
    } catch (err: any) {
      res.status(400).json({ message: "Invalid platform tax rule", error: err.message });
    }
  ...
  app.delete("/api/admin/tax-rules/platform/:id", ...mw, async (req: Request, res: Response) => {
    try {
      const result = await pool.query(`DELETE FROM tax_settings WHERE id = $1 RETURNING id`, [req.params.id]);
      if (!result.rows.length) return res.status(404).json({ message: "Platform tax rule not found" });
      res.status(204).end();
    } catch (err: any) {
      res.status(500).json({ message: "Failed to delete platform tax rule", error: err.message });
    }
  });

  app.post("/api/admin/tax-rules/preview", ...mw, async (req: Request, res: Response) => {
    try {
      const {
        serviceSubtotal = 0,
        platformSubtotal = 0,
      ...
      const result = calculateTaxBreakdown({
        serviceSubtotal: Number(serviceSubtotal),
        platformSubtotal: Number(platformSubtotal),
        discount: Number(discount),
        serviceTaxRatePercent: Number(serviceTaxRatePercent),
        platformTaxRatePercent: Number(platformTaxRatePercent),
      });
      res.json(result);
    } catch (err: any) {
      res.status(400).json({ message: "Invalid tax preview", error: err.message });
    }
  });
