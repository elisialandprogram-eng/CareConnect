...
export function registerAdminFinancialRoutes(app: Express): void {
          ...
          COALESCE((SELECT SUM(COALESCE(pe.gross_provider_payout_usd, pe.provider_earning, 0))
                    ...
                    WHERE pe.provider_id = p.id
                      AND COALESCE(pe.payment_method, ea.payment_method, 'card')
                            NOT IN ('cash', 'bank_transfer')), 0) AS settlement_gross_provider_payout,
  ...
  app.get("/api/admin/financial/providers/:providerId/detail", authenticateToken, requireAdmin, requirePermission(PERMISSIONS.PAYMENTS_VIEW), async (req: AuthRequest, res: Response) => {
    try {
      ...
      const [providerRow, summaryRow, monthlyRows, visitTypeRows, earningRows, walletRow, paidPayoutRow] = await Promise.all([
        ...
        // Use the actual provider_earnings.status / paid_at / payout_reference
        // so the UI shows Paid for earnings that have already been settled.
        pool.query(`
          SELECT
            a.id,
            ...
            COALESCE(pe.total_amount::text, COALESCE(a.final_total_usd, a.total_amount)::text) AS total_amount,
            a.created_at,
            pe.paid_at,
            pe.payout_reference,
  ...
  app.post("/api/admin/financial/providers/:providerId/mark-paid", authenticateToken, requireAdmin, requirePermission(PERMISSIONS.PAYMENTS_MANAGE), async (req: AuthRequest, res: Response) => {
    try {
      ...
      const { rows } = await pool.query(
        `SELECT pr.id, pr.provider_id, pr.status, p.country_code AS provider_country_code
         FROM payout_requests pr
         JOIN providers p ON p.id = pr.provider_id
         WHERE pr.id = $1 AND pr.provider_id = $2`,
        [payoutRequestId, providerId],
      );
      if (!rows[0]) return res.status(404).json({ message: "Payout request not found" });
      if (!canAccessCountry(req.user!, rows[0].provider_country_code)) {
        return res.status(404).json({ message: "Payout request not found" });
      }
      const client = await pool.connect();
      let transition: Awaited<ReturnType<typeof transitionProviderPayout>>;
      try {
        await client.query("BEGIN");
        transition = await transitionProviderPayout(
          client,
          payoutRequestId,
          "paid",
          req.user!.id,
          { paymentReference: paymentReference || null, adminNote: notes || null },
        );
      ...
      pool.query(
        ...
        [req.user!.id, payoutRequestId, JSON.stringify({ providerId, amount: transition.amountUsd, paymentReference }), req.user!.countryCode ?? null]
      ).catch(() => {});
      res.json({ ok: true, payout: transition.row, changed: transition.changed });
    } catch (err: any) {
    ...
    try {
      ...
      const { rows } = await pool.query(`
        SELECT
          ...
          COALESCE(NULLIF(TRIM(COALESCE(u.first_name,'') || ' ' || COALESCE(u.last_name,'')), ''), p.clinic_name, 'Unknown') AS provider_name,
          p.country_code,
          SUM(CASE WHEN a.payment_status = 'completed' THEN COALESCE(a.final_total_usd, a.total_amount)::numeric ELSE 0 END) AS gross_revenue,
          SUM(CASE WHEN a.payment_status = 'completed' AND a.total_amount::numeric > 0 THEN ROUND(COALESCE(a.platform_fee_amount::numeric, 0) / a.total_amount::numeric * COALESCE(a.final_total_usd, a.total_amount)::numeric, 4) ELSE 0 END) AS platform_fees,
          SUM(CASE WHEN a.payment_status = 'completed' AND a.total_amount::numeric > 0 THEN ROUND(COALESCE(a.refund_amount::numeric, 0) / a.total_amount::numeric * COALESCE(a.final_total_usd, a.total_amount)::numeric, 4) ELSE 0 END) AS total_refunds,
          COUNT(CASE WHEN a.payment_status = 'completed' THEN 1 END) AS completed_appointments
        FROM providers p
        JOIN users u ON u.id = p.user_id
        LEFT JOIN appointments a ON a.provider_id = p.id
  ...
  app.get("/api/admin/financial/reconciliation", authenticateToken, requireAdmin, async (req: AuthRequest, res: Response) => {
    try {
      ...
      const [revRow, feeRow, refundRow, payoutRow, pendPayoutRow, walletRow] = await Promise.all([
        pool.query(`SELECT COALESCE(SUM(COALESCE(final_total_usd, total_amount)::numeric), 0) AS v FROM appointments WHERE (payment_status='completed' OR status='completed') AND payment_status NOT IN ('refunded','failed') AND ($1::text IS NULL OR country_code::text=$1)`, [cc]),
        pool.query(`SELECT COALESCE(SUM(CASE WHEN total_amount::numeric > 0 THEN ROUND(COALESCE(platform_fee_amount::numeric,0) / total_amount::numeric * COALESCE(final_total_usd, total_amount)::numeric, 4) ELSE 0 END), 0) AS v FROM appointments WHERE (payment_status='completed' OR status='completed') AND payment_status NOT IN ('refunded','failed') AND ($1::text IS NULL OR country_code::text=$1)`, [cc]),
        pool.query(`SELECT COALESCE(SUM(CASE WHEN total_amount::numeric > 0 THEN ROUND(COALESCE(refund_amount::numeric,0) / total_amount::numeric * COALESCE(final_total_usd, total_amount)::numeric, 4) ELSE 0 END), 0) AS v FROM appointments WHERE payment_status='refunded' AND ($1::text IS NULL OR country_code::text=$1)`, [cc]),
      ...
      const { rows } = await pool.query(`
        SELECT pe.*,
               ...
               ,COALESCE((SELECT SUM(COALESCE(pe.gross_provider_payout_usd, pe.provider_earning, 0))
                          ...
                          WHERE pe.provider_id = p.id
                            AND COALESCE(pe.payment_method, ea.payment_method, 'card')
                                  NOT IN ('cash', 'bank_transfer')), 0) AS gross_provider_payout_usd
               ,COALESCE((SELECT SUM(COALESCE(pe.tax_pass_through_amount_usd, 0))
                          FROM provider_earnings pe
                          LEFT JOIN appointments ea ON ea.id = pe.appointment_id
                          WHERE pe.provider_id = p.id
               ...
               ,COALESCE((SELECT SUM(COALESCE(pe.settlement_amount_usd, pe.provider_earning, 0))
                          ...
                          WHERE pe.provider_id = p.id
                            AND COALESCE(pe.payment_method, ea.payment_method, 'card')
                                  NOT IN ('cash', 'bank_transfer')), 0) AS settlement_amount_usd
        FROM provider_earnings pe
  ...
  app.get("/api/admin/payout-requests", authenticateToken, requireAdmin, async (req: AuthRequest, res: Response) => {
    try {
      ...
      res.json(rows.map((row) => ({
        ...row,
        provider_name: [row.first_name, row.last_name].filter(Boolean).join(" ") || row.clinic_name || "Unknown provider",
        reviewed_by_name: [row.reviewed_by_first_name, row.reviewed_by_last_name].filter(Boolean).join(" ") || null,
        requested_amount_usd: row.amount,
  ...
  // Cash/bank-transfer platform-fee settlement report. Values come from the
  // immutable provider-earning snapshots, not recalculated appointments.
  app.get("/api/admin/cash-fee-settlements", authenticateToken, requireAdmin, requirePermission(PERMISSIONS.PAYMENTS_VIEW), async (req: AuthRequest, res: Response) => {
    try {
      const countryFilter = listingCountryFilter(req.user!, req.query as any);
      const params: any[] = [];
      const conditions = [`pe.payment_method IN ('cash', 'bank_transfer')`];
      let i = 1;
      if (countryFilter) { conditions.push(`p.country_code::text = $${i++}`); params.push(countryFilter); }
      if (req.query.dateFrom) { conditions.push(`COALESCE(a.date::text, pe.created_at::date::text) >= $${i++}`); params.push(String(req.query.dateFrom)); }
      if (req.query.dateTo) { conditions.push(`COALESCE(a.date::text, pe.created_at::date::text) <= $${i++}`); params.push(String(req.query.dateTo)); }
      ...
      console.error("[GET /api/admin/cash-fee-settlements]", err);
      res.status(500).json({ message: "Failed to load cash-fee settlements" });
    }
  });

  app.get("/api/admin/cash-fee-settlements/export", authenticateToken, requireAdmin, requirePermission(PERMISSIONS.PAYMENTS_VIEW), async (req: AuthRequest, res: Response) => {
    try {
      const countryFilter = listingCountryFilter(req.user!, req.query as any);
      const params: any[] = [];
      const conditions = [`pe.payment_method IN ('cash', 'bank_transfer')`];
      ...
      res.status(500).json({ message: "Failed to export cash-fee settlements" });
    }
  });
  ...
  app.patch("/api/admin/payout-requests/:id", authenticateToken, requireAdmin, requirePermission(PERMISSIONS.PAYMENTS_MANAGE), async (req: AuthRequest, res: Response) => {
    const { status, paymentReference, notes, adminNote } = req.body as { status: string; paymentReference?: string; notes?: string; adminNote?: string };
    const validStatuses = ["approved", "paid", "rejected"];
    if (!validStatuses.includes(status)) {
      return res.status(400).json({ message: "status must be one of: approved, paid, rejected" });
    }
    try {
      ...
      if (!previewRows[0]) return res.status(404).json({ message: "Payout request not found" });
      const preview = previewRows[0];
      if (!canAccessCountry(req.user!, preview.provider_country_code)) {
        return res.status(404).json({ message: "Payout request not found" });
      }
      ...
      try {
        ...
        transition = await transitionProviderPayout(
          ...
          req.user!.id,
          {
            ...
            adminNote: adminNote ?? notes ?? null,
          },
        );
        await client.query("COMMIT");
      } catch (transitionError) {
      ...
      const payout = transition.row;
      const previousStatus = transition.previousStatus;

      pool.query(
        `INSERT INTO audit_logs (user_id, action, entity_type, entity_id, details, country_code)
         VALUES ($1, 'payout_status_update', 'payout_request', $2, $3, $4)`,
        [req.user!.id, req.params.id, JSON.stringify({ status, paymentReference, prevStatus: previousStatus }), payout.country_code ?? preview.provider_country_code]
      ).catch(() => {});

      // Notify the provider via all channels (in-app + email + push) when payout status changes.
      if (previousStatus !== status && ["approved", "paid", "rejected"].includes(status)) {
        ...
        const _payoutCurr = countryCurrency((payout.country_code ?? preview.provider_country_code) as any) ?? "USD";
      ...
      res.json({ ...payout, changed: transition.changed });
    } catch (err: any) {
...
<body>
    ...
    try {
      ...
      const { rows } = await pool.query(`
        SELECT
          ...
          a.date,
          COALESCE(a.final_total_usd, a.total_amount)::text AS total_amount,
          a.payment_status,
          a.visit_type,
          a.country_code::text,
          a.created_at
      ...
      try {
        ...
        const { rows } = await pool.query(
          `SELECT
             ...
             COALESCE(SUM(CASE WHEN a.payment_status = 'completed' AND a.total_amount::numeric > 0 THEN ROUND(COALESCE(a.platform_fee_amount::numeric, 0) / a.total_amount::numeric * COALESCE(a.final_total_usd, a.total_amount)::numeric, 4) ELSE 0 END), 0) AS fees_usd,
             COALESCE(SUM(CASE WHEN a.payment_status = 'completed' AND a.total_amount::numeric > 0 THEN ROUND(COALESCE(a.refund_amount::numeric, 0) / a.total_amount::numeric * COALESCE(a.final_total_usd, a.total_amount)::numeric, 4) ELSE 0 END), 0) AS refunds_usd
           FROM appointments a
          ...
          const [promoRows, packageRows, referralRows, waitlistRows, giftCardRows] = await Promise.all([
            ...
            client.query(
              `SELECT
                 ...
                 COALESCE(SUM(COALESCE(a.final_total_usd, a.total_amount)::numeric), 0) AS gross_revenue_usd,
                 COALESCE(SUM(CASE WHEN a.total_amount::numeric > 0 THEN ROUND(COALESCE(a.promo_discount::numeric,0) / a.total_amount::numeric * COALESCE(a.final_total_usd, a.total_amount)::numeric, 4) ELSE 0 END), 0) AS total_discount_usd
               FROM promo_codes pc
            ...
            // Package conversion — revenue uses user_packages.price_paid (USD canonical)
            // not packages.price (native currency) to avoid cross-currency aggregation.
            client.query(
              `SELECT
                 ...
                 COALESCE(SUM(up.price_paid::numeric) FILTER (WHERE up.status != 'cancelled'), 0) AS total_revenue_usd
               FROM packages mp
  ...
  app.get(
    ...
    requireAdmin,
    async (req: AuthRequest, res: Response) => {
      const countryFilter = listingCountryFilter(req.user!, req.query as Record<string, any>);
      const client = await pool.connect();
      try {
