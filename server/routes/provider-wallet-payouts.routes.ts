    ...
    try {
      ...
      const rows = result.rows.map((r: any) => [
        ...
        r.patient_paid ?? "0",
         r.platform_fee ?? "0",
         r.platform_fee ?? "0",
          r.service_earnings_amount_usd ?? r.net_earning ?? "0",
          r.tax_pass_through_amount_usd ?? "0",
          r.cash_platform_fee_deduction_usd ?? "0",
          r.gross_provider_payout_usd ?? r.net_earning ?? "0",
          r.settlement_amount_usd ?? r.net_earning ?? "0",
         r.payment_method ?? "",
        r.currency ?? "USD",
        r.status ?? "",
          ...
          COALESCE((
           ...
           COALESCE((
             SELECT SUM(GREATEST(0,
               COALESCE(pe.gross_provider_payout_usd, pe.provider_earning::numeric, 0)
