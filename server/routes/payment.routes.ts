...
export function registerPaymentRoutes(app: Express): void {
  ...
  app.post("/api/gift-cards/purchase", authenticateToken, async (req: AuthRequest, res: Response) => {
    try {
      ...
      // Gift cards are purchased with the internal USD wallet. The card
      // creation and wallet debit must commit or roll back together.
      ...
      try {
        ...
        if (!wallet || wallet.currency !== "USD" || Number(wallet.balance) < amount) {
          throw Object.assign(new Error("Insufficient wallet balance"), { statusCode: 400 });
        }
        const nextBalance = round2(Number(wallet.balance) - amount).toFixed(2);
        await client.query(
        ...
        await client.query(
          `INSERT INTO wallet_transactions
           ...
           VALUES ($1, $2, 'debit', 'completed', $3, $4, 'USD', $5, 'gift_card', $6, $7)`,
          [
            wallet.id, req.user!.id, (-round2(amount)).toFixed(2), nextBalance,
            `Gift card purchase — ${code}`, code, `gift-card:${code}:purchase`,
          ],
        );
  ...
  app.post("/api/gift-cards/redeem", authenticateToken, async (req: AuthRequest, res: Response) => {
    ...
    try {
      ...
      const idempotencyKey = `gift-card:${card.id}:redeem`;
      const existingTx = await client.query(
        `SELECT balance_after, amount FROM wallet_transactions WHERE idempotency_key = $1 LIMIT 1`,
        [idempotencyKey],
      );
      if (existingTx.rows[0]) {
        ...
        return res.json({
          ok: true,
          amount: Math.abs(Number(existingTx.rows[0].amount)),
          newWalletBalance: Number(existingTx.rows[0].balance_after),
        });
      }
      if (!card.is_active) {
        throw Object.assign(new Error("This gift card has already been used"), { statusCode: 400 });
      }
      if (Number(card.balance) <= 0) {
        throw Object.assign(new Error("Gift card has no remaining balance"), { statusCode: 400 });
      }
      if (card.expires_at && new Date(card.expires_at) < new Date()) {
        throw Object.assign(new Error("This gift card has expired"), { statusCode: 400 });
      }
      ...
      if (!wallet || wallet.currency !== "USD") {
        throw Object.assign(new Error("Wallet credit could not be applied"), { statusCode: 400 });
      }
      const nextBalance = round2(Number(wallet.balance) + amount).toFixed(2);
      await client.query(
        `UPDATE wallets SET balance = $1, updated_at = NOW() WHERE id = $2`,
      ...
      await client.query(
        `INSERT INTO wallet_transactions
           (wallet_id, user_id, type, status, amount, balance_after, currency,
            description, reference_type, reference_id, idempotency_key)
         VALUES ($1, $2, 'topup', 'completed', $3, $4, 'USD', $5, 'gift_card', $6, $7)`,
        [
          wallet.id, req.user!.id, round2(amount).toFixed(2), nextBalance,
          `Gift card redeemed — ${card.code}`, card.id, idempotencyKey,
        ],
      );
      const redeemed = await client.query(
        `UPDATE gift_cards
            SET balance = 0, is_active = false, redeemed_by_user_id = $1, redeemed_at = NOW()
      ...
      await client.query("COMMIT");
      res.json({ ok: true, amount, newWalletBalance: Number(nextBalance) });
    } catch (e: any) {
      await client.query("ROLLBACK").catch(() => {});
      console.error("[gift-card/redeem]", e?.message);
      res.status(e?.statusCode ?? 500).json({ message: e?.statusCode ? e.message : "Failed to redeem gift card" });
    } finally {
      client.release();
    }
  });
  ...
  app.get("/api/invoices/me", authenticateToken, async (req: AuthRequest, res: Response) => {
    try {
      ...
      if (req.user.role === "provider") {
        ...
        const invoices = await storage.getInvoicesByProvider(provider.id);
        return res.json(invoices);
      }
      return res.json([]);
    } catch (error) {
