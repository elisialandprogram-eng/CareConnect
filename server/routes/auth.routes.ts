...
import { hashRefreshToken, issueAuthTokens } from "../lib/auth-tokens";
import {
  resend,
  FROM_EMAIL,
  hashOtp,
...
export function registerAuthRoutes(app: Express): void {
  ...
  app.post("/api/auth/login", authLimiter, async (req: Request, res: Response) => {
    try {
      ...
      await pool.query(`UPDATE users SET session_revoked_at = NULL WHERE id = $1`, [user.id]);
  ...
  app.post("/api/auth/logout", async (req: Request, res: Response) => {
    try {
      ...
      if (refreshToken) {
        await storage.deleteRefreshTokenByHash(hashRefreshToken(refreshToken));
      }
      // Access tokens are stateless, so revoke the session timestamp as well
      // as deleting the refresh token. This makes logout effective immediately
      // for every access token issued before the logout.
      if (userId) {
  ...
  app.post("/api/auth/refresh", async (req: Request, res: Response) => {
    try {
      ...
      if (!refreshToken) {
       ...
       // Consume the token in the same statement that validates expiry. This
       // makes two concurrent refresh requests unable to both rotate it.
       ...
       const storedToken = consumed.rows[0];
       if (!storedToken) {
        return res.status(401).json({ message: "Invalid or expired refresh token" });
      }

       const user = await storage.getUser(storedToken.user_id);
      if (!user) {
        return res.status(401).json({ message: "User not found" });
      }
       if (
         (user as any).sessionRevokedAt &&
         new Date((user as any).sessionRevokedAt) >= new Date(storedToken.created_at)
       ) {
         return res.status(401).json({ message: "Session revoked" });
       }
       if (user.isSuspended || !user.isEmailVerified) {
         return res.status(403).json({ message: "Account is not eligible for authentication" });
       }

       await pool.query(`UPDATE users SET session_revoked_at = NULL WHERE id = $1`, [user.id]);
       const { accessToken } = await issueAuthTokens(res, user, { mfaVerified: true });
  ...
  app.post("/api/auth/reset-password", authenticateToken, async (req: AuthRequest, res: Response) => {
    try {
      ...
      // Password changes invalidate every access and refresh token issued
      // before the change, including sessions on other devices.
  ...
  app.post("/api/auth/complete-reset-password", async (req: Request, res: Response) => {
    try {
      ...
      // A password reset is a credential compromise boundary: revoke all
      // existing sessions before allowing the user to sign in again.
      await pool.query(`UPDATE users SET session_revoked_at = NOW() WHERE id = $1`, [user.id]);
      await storage.deleteRefreshTokensByUser(user.id);
      invalidateAuthCache(user.id);
