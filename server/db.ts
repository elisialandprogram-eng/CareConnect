...
export async function runStartupMigrations() {
  ...
  for (const [label, statement] of providerWalletLedgerMigrations) {
    try {
      ...
      console.warn(`[db] ${label} migration warning:`, (err as Error).message);
    }
  }
  ...
  try {
    ...
    // Manual override flag — lets admins pin a rate that won't be touched by cron sync
    await pool.query(`ALTER TABLE currency_rates ADD COLUMN IF NOT EXISTS is_manual_override BOOLEAN NOT NULL DEFAULT false`);
  ...
  // Keep legacy plaintext cleanup independent from token_hash creation. Some
  // hosted Postgres installations also expose auth.refresh_tokens; this
  // unqualified public-table migration must never touch or depend on it.
  try {
    await pool.query(`ALTER TABLE public.refresh_tokens DROP COLUMN IF EXISTS token`);
    console.log("[db] refresh_tokens plaintext column removed");
  } catch (rtCleanupErr) {
    console.warn("[db] refresh_tokens plaintext cleanup warning (non-fatal):", (rtCleanupErr as Error).message);
  }
...
async function seedRbacRoles(): Promise<void> {
  ...
  try {
    ...
    await pool.query(`ALTER TABLE provider_schedule_templates ALTER COLUMN buffer_after_mins SET DEFAULT 5`);
  ...
  try {
    ...
    await pool.query(`ALTER TABLE appointments ADD COLUMN IF NOT EXISTS pricing_calculated_at      TIMESTAMPTZ`);
  ...
  // Phase 3: once pricing is persisted, the booking is the immutable
  // financial contract. Status, scheduling, payment, and refund fields may
  // still change, but price/tax/fee/currency fields may not.
  try {
    await pool.query(`
      CREATE OR REPLACE FUNCTION prevent_booking_pricing_snapshot_mutation()
      RETURNS trigger AS $$
      BEGIN
        IF OLD.pricing_calculated_at IS NOT NULL AND (
          OLD.total_amount IS DISTINCT FROM NEW.total_amount OR
          OLD.platform_fee_amount IS DISTINCT FROM NEW.platform_fee_amount OR
          OLD.service_price_snapshot IS DISTINCT FROM NEW.service_price_snapshot OR
          OLD.promo_code IS DISTINCT FROM NEW.promo_code OR
          OLD.promo_discount IS DISTINCT FROM NEW.promo_discount OR
          OLD.tax_amount IS DISTINCT FROM NEW.tax_amount OR
          OLD.service_subtotal IS DISTINCT FROM NEW.service_subtotal OR
          OLD.service_tax_rate IS DISTINCT FROM NEW.service_tax_rate OR
          OLD.service_tax_amount IS DISTINCT FROM NEW.service_tax_amount OR
          OLD.platform_taxable_subtotal IS DISTINCT FROM NEW.platform_taxable_subtotal OR
          OLD.platform_tax_rate IS DISTINCT FROM NEW.platform_tax_rate OR
          OLD.platform_tax_amount IS DISTINCT FROM NEW.platform_tax_amount OR
          OLD.payment_gateway_fee_amount IS DISTINCT FROM NEW.payment_gateway_fee_amount OR
          OLD.admin_fee_amount IS DISTINCT FROM NEW.admin_fee_amount OR
          ...
          OLD.re_applied_rules IS DISTINCT FROM NEW.re_applied_rules OR
          OLD.booking_currency IS DISTINCT FROM NEW.booking_currency OR
          OLD.provider_currency IS DISTINCT FROM NEW.provider_currency OR
          OLD.patient_currency IS DISTINCT FROM NEW.patient_currency OR
          OLD.final_total_usd IS DISTINCT FROM NEW.final_total_usd OR
          OLD.pricing_engine_version IS DISTINCT FROM NEW.pricing_engine_version OR
          OLD.tax_engine_version IS DISTINCT FROM NEW.tax_engine_version OR
          OLD.pricing_calculated_at IS DISTINCT FROM NEW.pricing_calculated_at OR
          OLD.display_currency IS DISTINCT FROM NEW.display_currency OR
          OLD.display_amount IS DISTINCT FROM NEW.display_amount OR
          OLD.exchange_rate_used IS DISTINCT FROM NEW.exchange_rate_used
        ) THEN
    ...
    `);
    await pool.query(`
      DROP TRIGGER IF EXISTS appointments_pricing_snapshot_immutable ON appointments;
      CREATE TRIGGER appointments_pricing_snapshot_immutable
...
export async function runBookingPricingSnapshotMigration(): Promise<void> {
  ...
  await pool.query(`
    CREATE TABLE IF NOT EXISTS sub_service_tax_rules (
      id VARCHAR PRIMARY KEY DEFAULT gen_random_uuid(),
      sub_service_id VARCHAR NOT NULL REFERENCES sub_services(id) ON DELETE CASCADE,
      country_code TEXT NOT NULL,
  ...
  await pool.query(`
    ALTER TABLE appointments
      ADD COLUMN IF NOT EXISTS service_subtotal NUMERIC(10,2) DEFAULT 0,
      ADD COLUMN IF NOT EXISTS service_tax_rate NUMERIC(5,2) DEFAULT 0,
      ADD COLUMN IF NOT EXISTS service_tax_amount NUMERIC(10,2) DEFAULT 0,
      ADD COLUMN IF NOT EXISTS platform_taxable_subtotal NUMERIC(10,2) DEFAULT 0,
      ADD COLUMN IF NOT EXISTS platform_tax_rate NUMERIC(5,2) DEFAULT 0,
      ADD COLUMN IF NOT EXISTS platform_tax_amount NUMERIC(10,2) DEFAULT 0,
  ...
  await pool.query(`
    ...
    FOR EACH ROW EXECUTE FUNCTION prevent_booking_pricing_snapshot_mutation();
  `);
  console.log("[db] booking pricing snapshot migration ready");
}
...
export async function runProviderSettlementMigration(): Promise<void> {
  ...
  const columns = [
    ...
    ["provider_earnings", "cash_platform_tax_deduction_usd", "NUMERIC(14,2)"],
    ["provider_earnings", "cash_commission_deduction_usd", "NUMERIC(14,2)"],
    ["provider_earnings", "cash_platform_fee_reversed_usd", "NUMERIC(14,2) NOT NULL DEFAULT 0"],
    ["provider_earnings", "cash_platform_tax_reversed_usd", "NUMERIC(14,2) NOT NULL DEFAULT 0"],
    ["provider_earnings", "cash_commission_reversed_usd", "NUMERIC(14,2) NOT NULL DEFAULT 0"],
    ["provider_earnings", "cash_wallet_debit_reversed_usd", "NUMERIC(14,2) NOT NULL DEFAULT 0"],
    ["provider_earnings", "cash_wallet_debit_applied_usd", "NUMERIC(14,2) NOT NULL DEFAULT 0"],
    ...
    ["provider_earnings", "cash_platform_fee_payout_request_id", "VARCHAR"],
    ["provider_earnings", "gross_provider_payout_usd", "NUMERIC(14,2)"],
    ["provider_earnings", "settlement_amount_usd", "NUMERIC(14,2)"],
    ["provider_earnings", "payment_method", "TEXT"],
  ...
  try {
    ...
    console.warn("[db] provider settlement indexes:", err.message);
  }
  console.log("[db] provider settlement migration ready");
}
