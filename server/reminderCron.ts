  ...
  for (const appt of stale) {
    try {
      ...
      const transition = await storage.transitionAppointment(
        appt.id,
        { status: "expired", privateNote: newNote, updatedAt: new Date() } as any,
        {
      ...
      if (!transition.ok) {
        log(`reminderCron[expire]: transition skipped for appt ${appt.id}: ${transition.message}`);
        continue;
      }
...
async function cancelStaleConfirmed() {
  ...
  for (const appt of stale) {
    try {
      ...
      const transition = await storage.transitionAppointment(
        appt.id,
        { status: "cancelled", privateNote: newNote, updatedAt: new Date() } as any,
        {
          action: "cancel" as any,
          actorUserId: null,
          actorRole: null,
      ...
      if (!transition.ok) {
        log(`reminderCron[stale]: transition skipped for appt ${appt.id}: ${transition.message}`);
        continue;
      }
      if (appt.timeSlotId) {
        try {
    ...
    for (const row of due) {
      try {
        ...
        if (balance >= priceUSD) {
          ...
          getRates().then(rates => {
            const _renewCurr = countryCurrency(row.country_code as import("./middleware/country").CountryCode | null) || "USD";
            notify.membershipRenewed(row.user_id, {
              packageName: row.package_name,
              formattedAmount: formatSync(priceUSD, _renewCurr, rates),
...
export function startReminderCron() {
  // The shared scheduler owns both recurring timers. The initial delay keeps
  // the first DB-heavy run away from the migration connection release.
  scheduler.register({
    name: "tick_5min",
  ...
  log("reminderCron registered — shared scheduler owns 5-min / hourly ticks");
}
