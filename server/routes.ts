...
export async function registerRoutes(
  ...
  app.get("/health", async (_req, res) => {
    ...
    const ready = dbStatus === "ok" && readiness.status === "ready";
    ...
    res.status(code).json({
      ...
      ready,
      migrationsStartedAt: readiness.migrationsStartedAt,
      migrationsCompletedAt: readiness.migrationsCompletedAt,
  ...
  // Every administrative route gets the same authentication and coarse role
  // boundary before module-specific handlers run. Individual routes may add
  // requireGlobalAdmin or requirePermission for narrower operations.
