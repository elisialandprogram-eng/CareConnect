 ...
 * Legacy super-admins (`admin` / `global_admin`) remain compatible with the
 * pre-RBAC system. All other admin access is checked against an active
 * admin_assignment row.
 */
...
export function requirePermission(permission: Permission) {
  return async (req: any, res: Response, next: NextFunction): Promise<void> => {
    ...
    try {
      ...
      // A missing assignment is an explicit deny. Defaults seed the role
      // catalogue and role-permission rows; they are not hidden runtime grants.
      if (!roleName || !perms.has(permission)) {
