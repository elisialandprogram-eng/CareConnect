...
export function adminScopeFor(role: string | undefined | null): AdminScope {
  if (isGlobalAdminRoleName(role)) return "global"; // legacy "admin" treated as global
...
/**
 * Returns the canonical currency code for a given country.
 *   HU → HUF   (Hungarian Forint)
 *   IR → IRR   (Iranian Rial)
 *   default → USD
 */
export function countryCurrency(countryCode: CountryCode | null | undefined): string {
