  ...
  try {
    ...
    if (!cached || cached.expires < now) {
      ...
      cached = {
        ...
        sessionRevokedAt: (user as any).sessionRevokedAt
          ? new Date((user as any).sessionRevokedAt)
          : null,
        full,
    ...
    const issuedAtMs = typeof (decoded as any).iat === "number"
      ? (decoded as any).iat * 1000
      : 0;
    if (
      cached.sessionRevokedAt &&
      issuedAtMs <= cached.sessionRevokedAt.getTime()
    ) {
...
export const optionalAuth = async (
  ...
  if (finalToken) {
    try {
      ...
      // Optional authentication must never trust a role from a JWT whose
      // subject no longer exists in the database. Treat that request as
      // anonymous instead of hydrating an unverifiable identity.
      if (cached && cached.expires >= now) {
        const issuedAtMs = typeof (decoded as any).iat === "number"
          ? (decoded as any).iat * 1000
          : 0;
        const revoked = cached.sessionRevokedAt &&
          issuedAtMs <= cached.sessionRevokedAt.getTime();
        if (!revoked) {
          const adminScope = adminScopeFor(cached.role);
          req.user = {
            id: decoded.id,
            email: cached.full?.email ?? decoded.email,
            role: cached.role,
