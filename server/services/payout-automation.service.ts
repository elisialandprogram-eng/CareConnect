...
export async function runBatchPayout(
  ...
  for (const provider of eligible) {
    ...
    let appliedCashFeeUsd = 0;
    try {
      await client.query("BEGIN");

      // Lock the wallet
      await client.query(`SELECT available_balance FROM provider_wallets WHERE provider_id = $1 FOR UPDATE`, [provider.providerId]);
      const cashFeeApplication = await applyPendingCashFeeDeductions(client, provider.providerId, triggeredBy);
      appliedCashFeeUsd = cashFeeApplication.totalAppliedUsd;
      const current = await client.query(`
        SELECT
          pw.available_balance::numeric AS available_balance,
          COALESCE((SELECT SUM(amount::numeric) FROM payout_requests
                    WHERE provider_id = $1 AND status IN ('pending','approved')), 0) AS in_flight
        FROM provider_wallets pw WHERE pw.provider_id = $1
      `, [provider.providerId]);
      const availableAfterFee = Math.max(
        0,
        Number(current.rows[0]?.available_balance ?? 0) - Number(current.rows[0]?.in_flight ?? 0),
      );
      const requestedAmount = Math.min(payoutAmount, availableAfterFee);
      if (requestedAmount < provider.minimumAmountUsd) {
        await client.query("ROLLBACK");
        client.release();
        results.push({ providerId: provider.providerId, status: "skipped", amountUsd: 0, reason: `Below minimum after cash-fee settlement ($${provider.minimumAmountUsd})` });
        continue;
      }
      ...
      await client.query(
        `UPDATE provider_wallets SET
           ...
           updated_at = NOW()
         WHERE provider_id = $2`,
        [requestedAmount, provider.providerId]
      );
      ...
      await client.query(
        ...
        [provider.providerId, -requestedAmount, payoutRequestId, `Batch payout hold — ${batchId}`]
      );
    ...
    const actualPayoutAmount = payoutRequestId
      ? Number((await pool.query(`SELECT amount FROM payout_requests WHERE id = $1`, [payoutRequestId])).rows[0]?.amount ?? payoutAmount)
      : payoutAmount;
    if (provider.stripeAccountId && provider.payoutsEnabled) {
      try {
        const { transferId } = await transferToConnectedAccount(
          provider.providerId, actualPayoutAmount, payoutRequestId!,
        ...
        try {
          ...
          await transitionProviderPayout(settleClient, payoutRequestId!, "paid", triggeredBy, {
            paymentReference: transferId,
            stripeTransferId: transferId,
            adminNote: `Stripe Connect transfer ${transferId}`,
          });
        ...
        totalAmountUsd += actualPayoutAmount;
        results.push({ providerId: provider.providerId, status: "succeeded", amountUsd: actualPayoutAmount, payoutRequestId, transferId });
      } catch (stripeErr) {
        const rejectClient = await pool.connect();
        try {
          await rejectClient.query("BEGIN");
          await transitionProviderPayout(rejectClient, payoutRequestId!, "rejected", triggeredBy, {
            adminNote: (stripeErr as Error).message,
          });
          await rejectClient.query("COMMIT");
        } catch (rejectError) {
          await rejectClient.query("ROLLBACK").catch(() => {});
          console.warn("[payout-automation] failed to reject payout:", rejectError);
        } finally {
          rejectClient.release();
        }
        results.push({ providerId: provider.providerId, status: "failed", amountUsd: actualPayoutAmount, reason: `Stripe error: ${(stripeErr as Error).message}`, payoutRequestId });
      }
    } else {
      // Manual payout path — stays in approved/held state for admin to process
      results.push({ providerId: provider.providerId, status: "succeeded", amountUsd: actualPayoutAmount, payoutRequestId, reason: "Awaiting manual disbursement" });
      totalAmountUsd += actualPayoutAmount;
    }
...
export async function retryFailedPayout(
  ...
  try {
    ...
    try {
      ...
      await transitionProviderPayout(holdClient, payoutRequestId, "approved", adminUserId, {
        adminNote: "Retrying failed payout",
      });
      await holdClient.query("COMMIT");
    } catch (holdError) {
      await holdClient.query("ROLLBACK").catch(() => {});
      throw holdError;
    } finally {
      holdClient.release();
    }
    ...
    const paidClient = await pool.connect();
    try {
      await paidClient.query("BEGIN");
      await transitionProviderPayout(paidClient, payoutRequestId, "paid", adminUserId, {
        paymentReference: transferId,
        stripeTransferId: transferId,
      });
      await paidClient.query("COMMIT");
    } catch (paidError) {
      await paidClient.query("ROLLBACK").catch(() => {});
      throw paidError;
    } finally {
      paidClient.release();
    }
    ...
    // A retry re-holds funds before attempting Stripe. If the transfer fails,
    // return that hold through the same lifecycle helper so the wallet cannot
    // remain stuck in held_balance.
    const releaseClient = await pool.connect();
    try {
      await releaseClient.query("BEGIN");
      await transitionProviderPayout(releaseClient, payoutRequestId, "rejected", adminUserId, {
        adminNote: `Retry transfer failed: ${(err as Error).message}`,
      });
      await releaseClient.query("COMMIT");
    } catch (releaseError) {
      await releaseClient.query("ROLLBACK").catch(() => {});
      console.warn("[payout-retry] failed to release retry hold:", releaseError);
    } finally {
