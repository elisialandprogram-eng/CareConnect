...
await it("B2 — refresh_tokens.token_hash values are long enough to be hashes (not plaintext tokens)", async () => {
  const { rows } = await pool.query(
    `SELECT token_hash FROM public.refresh_tokens LIMIT 5`,
  );
...
await it("D1 — RBAC permissions table has all 7 system roles seeded", async () => {
  const { rows } = await pool.query(
    `SELECT DISTINCT ar.name AS role
...
await it("D3 — Expired JWT is rejected on a protected collection endpoint", async () => {
  const expired = jwt.sign(
    { id: "test-user", email: "test@x.com", role: "patient" },
    JWT_SECRET,
    { expiresIn: "-1s" },
  );
  const res = await fetch(`${BASE_URL}/api/appointments/patient`, {
...
await it("G1 — financial_alerts has alert_fingerprint column", async () => {
  const { rows } = await pool.query(
     `SELECT column_name FROM information_schema.columns
      WHERE table_schema = 'public' AND table_name = 'financial_alerts' AND column_name = 'alert_fingerprint'`,
  );
...
await it("G2 — financial_alerts has occurrence tracking columns", async () => {
  const { rows } = await pool.query(
    `SELECT column_name FROM information_schema.columns
     WHERE table_schema = 'public' AND table_name = 'financial_alerts'
...
await it("G3 — Duplicate fingerprint inserts update occurrence_count, not create new rows", async () => {
  ...
  await pool.query(
    `INSERT INTO financial_alerts
       (alert_type, severity, message, is_resolved, alert_fingerprint,
        check_type, status, first_detected_at, last_detected_at, occurrence_count)
     VALUES ('test_dedup', 'warning', 'Dedup test alert', false, $1,
             'test_dedup', 'open', NOW(), NOW(), 1)`,
    [fp],
  );
  ...
  const { rows } = await pool.query(
     `SELECT COUNT(*) AS cnt, MAX(occurrence_count) AS max_occ
      FROM public.financial_alerts WHERE alert_fingerprint = $1`,
    [fp],
  );
