...
export abstract class GroupSessionsMixin {
  ...
  async getAnalyticsStats(countryCode?: string): Promise<{
    ...
    // Revenue figures use COALESCE(final_total_usd, total_amount) for USD
    // normalisation.  total_amount is booking currency (HUF/IRR/USD); using it
    // raw would inflate figures for non-USD providers.
    // We use payment_status='completed' consistently for ALL revenue metrics.
    // Country filter uses the same parameterised cast pattern as admin routes.
    ...
    try {
      ...
      const revenueRes = await client.query<{
        ...
        `SELECT
           -- All revenue figures use final_total_usd (USD-normalised) with
           -- total_amount as a fallback for older rows that pre-date the column.
           COALESCE(SUM(COALESCE(final_total_usd, total_amount)::numeric), 0) AS total_revenue,
           -- Platform fee is stored in booking currency; normalise to USD via
           -- the same pro-rata ratio used in every other admin analytics query.
           COALESCE(SUM(
             CASE WHEN total_amount::numeric > 0
               THEN ROUND(COALESCE(platform_fee_amount::numeric, 0)
                          / total_amount::numeric
                          * COALESCE(final_total_usd, total_amount)::numeric, 4)
               ELSE 0 END
           ), 0) AS platform_fees,
           COALESCE(SUM(COALESCE(final_total_usd, total_amount)::numeric)
        ...
        `WITH months AS (
         ...
         rev AS (
           SELECT
             ...
             COALESCE(SUM(COALESCE(final_total_usd, total_amount)::numeric), 0) AS revenue
           FROM appointments
           WHERE payment_status = 'completed'
