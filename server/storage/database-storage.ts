...
function mapProviderEarningRow(row: any): ProviderEarning {
  return {
    id: row.id,
    ...
    platformFee: String(row.platform_fee ?? row.platformFee ?? "0"),
    providerEarning: String(row.provider_earning ?? row.providerEarning ?? "0"),
    status: row.status,
    paidAt: row.paid_at ?? row.paidAt ?? null,
    paidByUserId: row.paid_by_user_id ?? row.paidByUserId ?? null,
    payoutReference: row.payout_reference ?? row.payoutReference ?? null,
    displayCurrency: row.display_currency ?? row.displayCurrency ?? null,
    displayAmount: row.display_amount ?? row.displayAmount ?? null,
    exchangeRateUsed: row.exchange_rate_used ?? row.exchangeRateUsed ?? null,
...
export interface IStorage {
  ...
  searchProviders(opts: { q?: string; type?: string; city?: string; verifiedOnly?: boolean; approvedOnly?: boolean; minRating?: number; countryCode?: "HU" | "IR" | null; limit?: number; offset?: number; homeVisit?: boolean; online?: boolean; priceMin?: number; priceMax?: number }): Promise<{ rows: ProviderWithUser[]; total: number }>;
  ...
  /**
   * Atomically validate a lifecycle transition, lock the appointment row,
   * update it, and append its audit event. This is the only write path for
   * status changes outside appointment creation.
   */
  transitionAppointment(
    id: string,
    data: Partial<InsertAppointment>,
    event: Omit<InsertAppointmentEvent, "appointmentId">,
    options?: { allowAdminOverride?: boolean; allowNoop?: boolean },
  ): Promise<AppointmentTransitionResult>;

  // Reviews
  getReview(id: string): Promise<Review | undefined>;
  getReviewsByProvider(providerId: string, includePending?: boolean): Promise<ReviewWithPatient[]>;
...
export class DatabaseStorage extends PackagesMixin implements IStorage {
  ...
  async searchProviders(opts: { q?: string; type?: string; city?: string; verifiedOnly?: boolean; approvedOnly?: boolean; minRating?: number; countryCode?: "HU" | "IR" | null; limit?: number; offset?: number; homeVisit?: boolean; online?: boolean; priceMin?: number; priceMax?: number }): Promise<{ rows: ProviderWithUser[]; total: number }> {
    ...
    if (opts.q && opts.q.trim().length > 0) {
      ...
      if (opts.minRating && opts.minRating > 0) {
        ...
        structConds.push(`p.rating::numeric >= $${params.length}`);
      }
      // Visit modes and price are service-level facts. Keep them in the same
      // WHERE clause as the FTS query so pagination totals remain accurate.
      if (opts.homeVisit) {
        structConds.push(`EXISTS (
          SELECT 1 FROM services s
           WHERE s.provider_id = p.id AND s.is_active = true
             AND s.location_mode IN ('home_only','both','home_online','all')
        )`);
      }
      if (opts.online) {
        structConds.push(`EXISTS (
          SELECT 1 FROM services s
           WHERE s.provider_id = p.id AND s.is_active = true
             AND s.location_mode IN ('online_only','both','clinic_online','home_online','all')
        )`);
      }
      if ((opts.priceMin != null && Number.isFinite(opts.priceMin)) ||
          (opts.priceMax != null && Number.isFinite(opts.priceMax))) {
        const priceConds = ["s.provider_id = p.id", "s.is_active = true"];
        if (opts.priceMin != null && Number.isFinite(opts.priceMin)) {
          params.push(opts.priceMin);
          priceConds.push(`s.price::numeric >= $${params.length}`);
        }
        if (opts.priceMax != null && Number.isFinite(opts.priceMax)) {
          params.push(opts.priceMax);
          priceConds.push(`s.price::numeric <= $${params.length}`);
        }
        structConds.push(`EXISTS (SELECT 1 FROM services s WHERE ${priceConds.join(" AND ")})`);
      }
    ...
    if (opts.minRating && opts.minRating > 0) {
      conds.push(sql`${providers.rating}::numeric >= ${opts.minRating}`);
    }
    if (opts.homeVisit) {
    ...
    if ((opts.priceMin != null && Number.isFinite(opts.priceMin)) ||
      ...
      conds.push(sql`EXISTS (
        SELECT 1 FROM services s
         WHERE s.provider_id = ${providers.id} AND s.is_active = true
           ${opts.priceMin != null && Number.isFinite(opts.priceMin) ? sql`AND s.price::numeric >= ${opts.priceMin}` : sql``}
           ${opts.priceMax != null && Number.isFinite(opts.priceMax) ? sql`AND s.price::numeric <= ${opts.priceMax}` : sql``}
      )`);
    }
  ...
  async getProviderRevenueTotal(providerId: string): Promise<number> {
    // Use final_total_usd (USD-normalised) with total_amount as fallback for
    // pre-engine rows.  total_amount alone is booking currency (HUF/IRR/USD)
    // and would inflate figures for non-USD providers.
    // Note: final_total_usd is not in the Drizzle schema (added via raw migration),
    // so we must use raw pool.query here.
    const { rows } = await pool.query<{ total: string }>(
      `SELECT COALESCE(SUM(COALESCE(final_total_usd, total_amount)::numeric), 0) AS total
       FROM appointments
       WHERE provider_id = $1 AND payment_status = 'completed'`,
      [providerId],
    );
  ...
  async updateAppointment(id: string, data: Partial<InsertAppointment>): Promise<Appointment | undefined> {
    if ((data as any).status !== undefined) {
      throw new Error("Appointment status changes must use transitionAppointment()");
    }
  ...
  /**
   * Canonical appointment lifecycle write.
   *
   * The row is locked before checking the transition so two concurrent
   * requests cannot both validate against the same old status. The audit
   * event uses the status read under that lock, not a caller-supplied copy.
   */
  async transitionAppointment(
    ...
    const requestedStatus = String((data as any).status ?? "");
    if (!requestedStatus) {
      return { ok: false, status: 400, message: "Appointment status is required for a lifecycle transition." };
    }
    ...
    return await db.transaction(async (tx) => {
      ...
      if (current.status === requestedStatus) {
        if (options.allowNoop) {
          // Payment webhooks and retryable wallet flows may repeat a
          // confirmation after the appointment is already confirmed. Apply
          // the non-status snapshot fields, but do not create a duplicate
          // lifecycle event for a no-op.
          const nonStatusData = { ...(data as any) };
          delete nonStatusData.status;
          if (Object.keys(nonStatusData).length === 0) {
            return { ok: true, status: 200, appointment: current };
          }
          const [updatedNoop] = await tx
            .update(appointments)
            .set(nonStatusData)
            .where(eq(appointments.id, id))
            .returning();
          return { ok: true, status: 200, appointment: updatedNoop ?? current };
        }
        return {
          ok: false,
          status: 409,
      ...
      if (!options.allowAdminOverride && !canTransition(current.status, requestedStatus)) {
        return {
          ok: false,
          status: 409,
          message: `Cannot move appointment from ${current.status} to ${requestedStatus}.`,
          allowed: nextStatusesFor(current.status),
        };
      }
      ...
      const [createdEvent] = await tx
        ...
        .values({
          ...
          toStatus: requestedStatus as any,
        })
        .returning();

      return { ok: true, status: 200, appointment: updated, event: createdEvent };
    });
  }
  ...
  async getReviewsByProvider(providerId: string, includePending = false): Promise<ReviewWithPatient[]> {
    const result = await db
      .select()
      .from(reviews)
      ...
      .where(includePending
        ...
        : and(eq(reviews.providerId, providerId), eq(reviews.status, "approved")))
  ...
  async recomputeProviderRating(providerId: string): Promise<{ rating: string; totalReviews: number }> {
    const raw: any = await db.execute(
      sql`SELECT AVG(rating)::numeric(2,1) AS avg_rating, COUNT(*)::int AS total
          FROM reviews WHERE provider_id = ${providerId} AND status = 'approved'`
    );
    const row = Array.isArray(raw) ? raw[0] : raw?.rows?.[0];
  ...
  async recordProviderEarning(appointmentId: string): Promise<ProviderEarning | null> {
    ...
    try {
      ...
      // The unique appointment_id constraint is the idempotency key. A
      ...
      // Offline methods are eligible only after the provider/admin explicitly
      // records receipt. The completed payment row and the appointment
      ...
      if (!insertResult.rows[0]) {
        const raced = await client.query(`SELECT * FROM provider_earnings WHERE appointment_id = $1 FOR UPDATE`, [appointmentId]);
        await client.query("COMMIT");
      ...
      if (!settlement.isOffline) {
        ...
        ON CONFLICT (provider_id) DO UPDATE SET
          ...
          updated_at = NOW()
        `, [appt.provider_id, settlement.grossProviderPayoutUsd, countryCode]);
        const walletRes = await client.query(
        `SELECT available_balance FROM provider_wallets WHERE provider_id = $1 FOR UPDATE`,
        [appt.provider_id],
        );
        const balAfterIncome = Number(walletRes.rows[0]?.available_balance ?? 0);
        await client.query(`
        ...
        `, [
        appt.provider_id,
        settlement.grossProviderPayoutUsd,
        appointmentId,
        `Booking income — includes patient-paid tax — appt #${appt.appointment_number || appointmentId.slice(0, 8)}`,
        balAfterIncome,
        countryCode,
        ]);
      }
  ...
  async getEarningsSummary(providerId?: string): Promise<{
    ...
    patientPlatformFees: string;
    totalPlatformRevenue: string;
    count: number;
  }> {
    const where = providerId
      ? "WHERE pe.provider_id = $1 AND COALESCE(pe.payment_method, a.payment_method, 'card') NOT IN ('cash', 'bank_transfer')"
      : "WHERE COALESCE(pe.payment_method, a.payment_method, 'card') NOT IN ('cash', 'bank_transfer')";
