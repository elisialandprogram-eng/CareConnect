...
async function seed() {
  ...
  const allSubServices = [
    ...physiotherapistSubServices.map(s => ({ ...s, category: "rehabilitation" as const })),
    ...doctorSubServices.map(s => ({ ...s, category: "physician" as const })),
    ...nurseSubServices.map(s => ({ ...s, category: "nursing" as const }))
  ];
