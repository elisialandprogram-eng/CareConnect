...
16. **Round of UX fixes (Apr 2026)** — bundled fixes covering seven user-reported issues:
    - **Server error toasts**: `client/src/lib/queryClient.ts` `throwIfResNotOk` now parses JSON responses and surfaces the server's `message` field (with status attached), so failed mutations show the real reason instead of a generic blob.
    - **Currency stickiness**: `useCurrency` now reads `useAuth().user.preferredCurrency` first (then localStorage, then language). Hardcoded `formatHUF`/HUF strings replaced in `admin-earnings.tsx`, `provider-earnings.tsx`, `wallet.tsx`, `admin-dashboard.tsx`, `provider-dashboard.tsx`, and `profile.tsx` so changing currency in the profile updates everything instantly.
    - **Booking cache invalidation**: `book-wizard.tsx` `bookMut.onSuccess` now invalidates `/api/appointments/patient`, `/api/appointments/provider`, and `/api/wallet`, so new bookings appear immediately on the appointments and dashboard pages.
    - **Multi-session per-session slots**: replaced single `sessions: number` with `additionalSlots: Array<{date,startTime,endTime}>`. Wizard step 3 renders a date+time picker per extra session; server `POST /api/appointments` accepts the array and creates child appointments (`totalAmount=0`, `parentAppointmentId` set, `paymentMethod=bundled`). Conflict-or-failure on any extra slot rolls back parent + earlier children to `cancelled`.
    - **Service location mode**: added `services.location_mode` and `service_requests.location_mode` columns (text default `both`, values `clinic_only` | `home_only` | `both`). Provider request form, admin edit dialog, and admin approval all carry the value through. Booking wizard disables visit-type buttons that don't match. Server `POST /api/appointments` rejects mismatched bookings with a clear message.
    - **Lock down admin-assigned services**: services with a `subServiceId` (admin-assigned) hide Edit/Pricing/Delete in `provider-dashboard.tsx` and show a "Managed by Admin" hint. Server `PATCH /api/services/:id` rejects non-`isActive` field changes from providers (403), and `DELETE /api/services/:id` is blocked entirely for assigned services.
    - **Mandatory clinic address**: `POST /api/appointments` returns 400 when `visitType === "clinic"` and the provider's `primaryServiceLocation` is empty. Provider dashboard shows an amber banner above the Services tab when any clinic-mode service is active but no address is set, with a "Go to Profile" jump button. Appointments list cards now show the clinic address (for clinic visits) and the patient's home address (for home visits) directly on the card.

15. **Provider-driven service request workflow** — added a `service_requests` table (provider_id FK, category, service_name, sub_service_name, suggested_price, description, status `pending_review`/`approved`/`rejected`, admin_notes, rejection_reason, created_service_id) so providers can request brand-new service offerings without admin write access to the catalog. Storage methods: `createServiceRequest`, `getServiceRequest`, `listServiceRequestsByProvider`, `listAllServiceRequests` (joined with provider+user), `findPendingServiceRequest` (dedupe guard), `updateServiceRequest`. Endpoints: `POST/GET /api/provider/service-requests`, `GET /api/admin/service-requests`, `PATCH /api/admin/service-requests/:id` (admin pre-approval edits), `POST /api/admin/service-requests/:id/approve` (find-or-create matching `sub_services` catalog row, then create per-provider `services` row with admin-chosen duration + finalPrice), `POST /api/admin/service-requests/:id/reject` (with mandatory reason). All transitions push an in-app entry into `notification_queue` for the requesting provider (subjects "Service request submitted" / "approved" / "rejected"). UI: new **Request Service** tab on `provider-dashboard.tsx` (form + history list with status badge and admin notes/rejection reason) and new **Service Requests** tab on `admin-dashboard.tsx` with pending/approved/rejected filter + Edit / Approve / Reject dialogs. Duplicate guard: another `pending_review` request from the same provider for the same `serviceName` returns 409.

17. **Group therapy / group sessions** — added two tables: `group_sessions` (provider_id, optional service_id, title, description, country_code, scheduled_at, duration_minutes, capacity, price + currency, video_link, status `scheduled`/`in_progress`/`completed`/`cancelled`) and `group_session_participants` (session_id, user_id UNIQUE per session, status `booked`/`attended`/`no_show`/`cancelled`, paid_amount/currency, payment_id, joined_at, refunded_at). Storage methods on `IStorage`: `createGroupSession`, `listGroupSessionsByCountry`, `listGroupSessionsByProvider`, `getGroupSessionWithParticipants`, `updateGroupSession`, `bookGroupSessionWithWallet` (single transaction with `FOR UPDATE` row-lock, capacity + country + dup checks, deterministic idempotency key `group_book_<sid>_<uid>` and inlined wallet debit that populates wallet_id/balance_after/currency), `cancelGroupSessionAndRefund` (refunds every booked participant with idempotency key `group_refund_<sid>_<uid>` so re-runs are safe), `markGroupParticipantAttendance` (provider-scoped via SQL join), `recordGroupSessionJoin` (15-min-before / 30-min-after window), `listMyGroupBookings`, and `tickGroupSessionStatuses` (called from the 5-min `reminderCron` tick to flip scheduled→in_progress→completed). `migrateUserCountry` was extended to also move `group_session_participants` rows so country switches stay consistent. Endpoints: `POST/GET/PATCH /api/provider/group-sessions`, `POST /api/provider/group-sessions/:id/cancel`, `POST /api/provider/group-sessions/:id/participants/:pid/attendance`, `GET /api/group-sessions` (country-scoped public list, hides cancelled), `POST /api/group-sessions/:id/book`, `POST /api/group-sessions/:id/join`, `GET /api/me/group-sessions`. Country is **forced** to provider.countryCode on create; PATCH strips status/country/providerId. Payment is tracked on the participant row (since `payments.appointment_id` is NOT NULL — we don't fake an appointment per booking). UI: provider dashboard gets a new **Group Sessions** tab (`client/src/components/group-sessions-panel.tsx` — create dialog, sessions list with cancel + participant management dialog with attendance select); patients get a new `/group-sessions` page (Browse + My bookings tabs) and a "Groups" link in the header. Audit suite (`script/audit/runner.ts` section 8d) gained 14 checks covering create-permission, country scope, capacity full, duplicate booking, wallet debit, refund-all, idempotent cancel, and delisting — all 14 pass (52/53 overall).
