...
export function GroupSessionsPanel() {
  ...
  const toUSD = (v: string): string => {
    if (!v || code === "USD") return v;
    const n = Number(v);
    if (!Number.isFinite(n) || n === 0) return v;
    return String(Number(convertBetweenCurrencies(n, code as SupportedCurrency, "USD").toFixed(6)));
  };
  ...
  const create = useMutation({
    mutationFn: async () => {
      const payload = {
        ...
        pricePerUser: toUSD(String(form.pricePerUser)),
