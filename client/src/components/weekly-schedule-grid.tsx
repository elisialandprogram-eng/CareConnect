...
function gridFromSchedule(rawSchedule: any): GridState {
  const grid = emptyGrid();
  let schedule: Partial<WeeklySchedule>;
  if (typeof rawSchedule === "string") {
    try { schedule = JSON.parse(rawSchedule); } catch { return grid; }
  } else {
  ...
  DAY_KEYS.forEach((d) => {
    ...
    // Use windows array when present (supports breaks), else fall back to start/end
    const windows = Array.isArray(entry.windows) && entry.windows.length > 0
      ? entry.windows
      : [{ start: entry.start, end: entry.end }];
    for (const win of windows) {
      const startBlock = Math.max(0, timeToBlock(win.start));
      const endBlock = Math.min(BLOCK_COUNT, timeToBlock(win.end));
...
export function WeeklyScheduleGrid({
  ...
  const handlePublish = async () => {
    ...
    // Use local-date formatting to avoid UTC midnight shift in UTC+ timezones
    // (toISOString converts to UTC and can roll back one calendar day in e.g. UTC+2)
  ...
  return (
    <div className="space-y-4" data-testid={readOnly ? "weekly-schedule-grid-preview" : "weekly-schedule-grid"}>
      {readOnly && (
        <p className="text-xs text-muted-foreground">Visual preview of your weekly schedule</p>
      )}
      {/* Quick fill toolbar — hidden in readOnly mode */}
      {!readOnly && (
                ...
                <tr key={blockIdx} className={isHourStart ? "border-t border-border/40" : ""}>
                  ...
                  {DAY_KEYS.map((d) => {
                    ...
                    return (
                      <td
                        key={d}
                        className={`border-l border-border/20 p-0 transition-colors duration-75 ${
                          readOnly ? "cursor-default" : "cursor-pointer"
                        } ${
                          on
                            ? readOnly ? "bg-primary/60" : "bg-primary/70 hover:bg-primary/90"
                            : readOnly ? "" : "hover:bg-primary/10"
                        } ${isHourStart ? "border-t border-border/30" : ""}`}
                        style={{ height: 16 }}
                        onMouseDown={readOnly ? undefined : () => onMouseDown(d, blockIdx)}
                        onMouseEnter={readOnly ? undefined : () => onMouseEnter(d, blockIdx)}
      ...
      {/* Per-day quick actions — hidden in readOnly mode */}
      {!readOnly && (
        ...
        <>
          ...
          <div className="rounded-xl border border-border bg-muted/20 p-4 space-y-3">
            ...
            >
              ...
              {previewLoading ? "Checking conflicts…" : "Publish this week's slots"}
            </Button>
          </div>
