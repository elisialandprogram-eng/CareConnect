...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const login = async (email: string, password: string): Promise<{ mfaRequired: true; mfaToken: string } | void> => {
    ...
    // MFA challenge — caller must show TOTP screen
  ...
  const register = async (data: RegisterData) => {
    ...
    // Return full result so callers can detect verification_required (Case B)
    // or read the new user from result.user (Case new registration).
    return result;
  };
