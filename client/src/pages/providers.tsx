...
export default function Providers() {
  ...
  // Rating is a server-side filter so totals and pagination remain accurate.
  useEffect(() => {
    setDebouncedQuery((d) => ({ ...d, minRating: filters.minRating }));
    setPage(1);
  }, [filters.minRating]);
  ...
  useEffect(() => {
    ...
    setPage(1);
  }, [filters.homeVisit, filters.online, filters.priceRange]);
  ...
  const queryString = (() => {
    ...
    if (debouncedQuery.minRating > 0) sp.set("minRating", String(debouncedQuery.minRating));
    if (debouncedQuery.homeVisit) sp.set("homeVisit", "true");
    if (debouncedQuery.online) sp.set("online", "true");
    if (debouncedQuery.priceMin > 0) sp.set("priceMin", String(debouncedQuery.priceMin));
    if (debouncedQuery.priceMax < 500) sp.set("priceMax", String(debouncedQuery.priceMax));
  ...
  const { data: result, isLoading } = useQuery<ProvidersPage>({
    queryKey: QK.providerSearch(debouncedQuery.type, debouncedQuery.q, debouncedQuery.location, debouncedQuery.verifiedOnly, debouncedQuery.minRating, debouncedQuery.homeVisit, debouncedQuery.online, debouncedQuery.priceMin, debouncedQuery.priceMax, page),
    queryFn: async () => {
  ...
  // Visit modes and price ranges are applied by the API before pagination.
  // Keep this list unfiltered so the displayed total matches the page.
