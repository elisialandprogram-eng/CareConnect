...
export default function SupportTicketsPage() {
  ...
  const { data: detail, isLoading: detailLoading } = useQuery<TicketDetail>({
    queryKey: QK.supportTicket(selectedId as string),
    enabled: !!selectedId,
  ...
  const replyMutation = useMutation({
    ...
    onSuccess: () => {
      setReply("");
      queryClient.invalidateQueries({ queryKey: QK.supportTicket(selectedId as string) });
      queryClient.invalidateQueries({ queryKey: QK.supportTickets() });
    },
    onError: (e: Error) => {
      showErrorModal({
  ...
  return (
    <div className="min-h-screen flex flex-col">
      <Header />
      <PageBreadcrumbs items={[{ label: t("common.support", "Support") }]} />
