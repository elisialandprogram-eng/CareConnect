...
import { AvatarLG } from "@/components/ui/provider-image";
...
const emptyForm: FormState = {
  ...
  placeId: "", formattedAddress: "", savedLatitude: null, savedLongitude: null,
  ...
  yearsExperience: 0, licenseNumber: "",
};
...
function validateProfile(data: FormState): FormErrors {
  const errors: FormErrors = {};
  if (!data.firstName.trim()) errors.firstName = "First name is required.";
  if (!data.lastName.trim()) errors.lastName = "Last name is required.";
  if (data.phone && !PHONE_RE.test(data.phone.trim()))
    errors.phone = "Enter a valid phone number (digits, spaces, + and - allowed).";
  if (!data.mobileNumber.trim())
    errors.mobileNumber = "Mobile number is required.";
  else if (!PHONE_RE.test(data.mobileNumber.trim()))
    errors.mobileNumber = "Enter a valid mobile number.";
  if (!data.emergencyContactName.trim())
    errors.emergencyContactName = "Emergency contact name is required.";
  if (!data.emergencyContactPhone.trim())
    errors.emergencyContactPhone = "Emergency contact phone is required.";
  else if (!PHONE_RE.test(data.emergencyContactPhone.trim()))
  ...
  if (data.heightCm !== "") {
    ...
    if (!Number.isFinite(h) || h < 30 || h > 300)
      errors.heightCm = "Height must be between 30 and 300 cm.";
  }
  if (data.weightKg !== "") {
    const w = Number(data.weightKg);
    if (!Number.isFinite(w) || w < 0.5 || w > 700)
      errors.weightKg = "Weight must be between 0.5 and 700 kg.";
  }
  return errors;
}

// Which tab owns each field (for auto-switching on error)
const FIELD_TAB: Partial<Record<keyof FormState, string>> = {
  firstName: "personal", lastName: "personal",
  dateOfBirth: "personal", phone: "personal", mobileNumber: "personal",
  heightCm: "medical", weightKg: "medical",
...
export default function Profile() {
  ...
  useEffect(() => {
    if (user) {
      ...
      setFormData({
        ...
        savedLatitude: u.savedLatitude ?? null,
        savedLongitude: u.savedLongitude ?? null,
        gender: u.gender || "",
  ...
  const updateProfileMutation = useMutation({
    mutationFn: async (data: FormState) => {
      ...
      const payload: any = {
        ...
        savedLatitude: data.savedLatitude,
        savedLongitude: data.savedLongitude,
        ...
        // Send empty string instead of null so the server normalises it cleanly
    ...
    onSuccess: (_data, variables: any) => {
      // Refresh the auth context user (useState) so the form re-populates with
      // saved values instead of reverting to stale pre-save state.
      void refreshUser();
      queryClient.invalidateQueries({ queryKey: QK.authMe() });
    ...
    onError: (err: any) => {
      showErrorModal({
        ...
        description: err?.message || t("profile_page.toast_profile_failed", "Failed to update profile. Please try again."),
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (Object.keys(errors).length > 0) {
      // Mark all errored fields as touched so they all turn red
      setTouched((prev) => {
        const next = new Set(prev);
        (Object.keys(errors) as (keyof FormState)[]).forEach((k) => next.add(k));
        return next;
      });
      // Auto-switch to the tab that contains the first error
      const TAB_ORDER = ["personal", "address", "medical", "insurance", "emergency"];
      const firstErrorField = (Object.keys(errors) as (keyof FormState)[]).find(
        (k) => FIELD_TAB[k],
      );
      const targetTab = firstErrorField ? FIELD_TAB[firstErrorField] : undefined;
      if (targetTab && TAB_ORDER.includes(targetTab)) setActiveTab(targetTab);
      return;
    }
  ...
  // Mark a field touched on blur and re-validate immediately
  const touch = (k: keyof FormState) => {
    ...
    setFormErrors((prev) => ({ ...prev, ...validateProfile(formData) }));
  };

  // Helper: error message for a field (only shown after it's been touched)
  const fieldError = (k: keyof FormState) =>
    touched.has(k) && formErrors[k] ? (
      <p className="text-xs text-destructive mt-1" data-testid={`error-${k}`}>{formErrors[k]}</p>
    ) : null;

  // Helper: add destructive border when field has a visible error
  const errClass = (k: keyof FormState) =>
    touched.has(k) && formErrors[k] ? "border-destructive focus-visible:ring-destructive" : "";
  ...
  const { completion, missingFields } = useMemo(() => {
    ...
    const fields: { value: string | undefined; label: string }[] = [
      ...
      { value: formData.mobileNumber, label: t("profile_page.mobile_number", "Mobile number") },
    ...
    <div className="min-h-screen flex flex-col bg-muted/30">
      ...
      <main className="flex-1 container mx-auto px-4 py-8 max-w-5xl">
                ...
                <div className="flex flex-wrap gap-2 mt-2">
                  <Button variant="outline" size="sm" asChild>
                    <Link href="/settings" data-testid="link-go-to-settings">
                      ...
                      {t("profile_page.go_to_settings", "Settings")}
                    </Link>
                  </Button>
                  {isProvider && (
                    <Button variant="outline" size="sm" asChild>
        ...
        <form onSubmit={handleSubmit}>
                  ...
                  <div className="grid sm:grid-cols-2 gap-4">
                    <div className="space-y-2">
                      ...
                      <div className="relative">
                        ...
                        <Input id="dateOfBirth" type="date" className={`pl-10 ${errClass("dateOfBirth")}`}
                  ...
                  <div className="grid sm:grid-cols-2 gap-4">
                    <div className="space-y-2">
                      ...
                      <div className="relative">
                        ...
                        <Input id="phone" type="tel" className={`pl-10 ${errClass("phone")}`}
                    ...
                    <div className="space-y-2">
                      <Label htmlFor="mobileNumber">
                        {t("profile_page.mobile_number", "Mobile Number")}
                        <span className="text-destructive ml-1">*</span>
                      </Label>
                      <div className="relative">
                        <Phone className="absolute start-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground pointer-events-none" />
                        <Input id="mobileNumber" type="tel" className={`pl-10 ${errClass("mobileNumber")}`}
                    ...
                    <div className="space-y-2">
                      ...
                      <PlacesAutocomplete
                        ...
                        onChange={(text, structured?: StructuredAddress) => {
                          set("address", text);
                          if (structured) {
                            if (structured.city) set("city", structured.city);
                            if (structured.state) set("state", structured.state);
                            if (structured.postalCode) set("zipCode", structured.postalCode);
                            if (structured.placeId) set("placeId", structured.placeId);
                            if (structured.formattedAddress) set("formattedAddress", structured.formattedAddress);
                            set("savedLatitude", structured.latitude ?? null);
                            set("savedLongitude", structured.longitude ?? null);
                          }
                        }}
                        placeholder={t("profile_page.address_placeholder", "Start typing your address…")}
                  ...
                  <CardContent className="space-y-5">
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                      ...
                      <div className="space-y-2">
                        ...
                        <Input id="heightCm" type="number" min={30} max={300} value={formData.heightCm}
                          onChange={(e) => set("heightCm", e.target.value)}
                          onBlur={() => touch("heightCm")}
                          className={errClass("heightCm")}
                          placeholder="170"
                      ...
                      <div className="space-y-2">
                        ...
                        <Input id="weightKg" type="number" min={0.5} max={700} step="0.1" value={formData.weightKg}
                          onChange={(e) => set("weightKg", e.target.value)}
                          onBlur={() => touch("weightKg")}
                          className={errClass("weightKg")}
                          placeholder="70"
                  ...
                  <div className="grid sm:grid-cols-2 gap-4">
                    <div className="space-y-2">
                      <Label htmlFor="emergencyContactName">
                        {t("profile_page.contact_name", "Contact Name")}
                        <span className="text-destructive ml-1">*</span>
                      </Label>
                    ...
                    <div className="space-y-2">
                      <Label htmlFor="emergencyContactPhone">
                        {t("profile_page.contact_phone", "Contact Phone")}
                        <span className="text-destructive ml-1">*</span>
                      </Label>
                      <Input id="emergencyContactPhone" type="tel"
                ...
                <Card>
                  ...
                  <CardContent className="space-y-5">
                    ...
                    <Button type="button" asChild>
                      <Link href="/provider/dashboard?tab=profile" data-testid="link-edit-provider">
                        <Settings className="h-4 w-4 mr-2" />
          ...
          {/* Sticky save bar (hidden on Security/Gallery/Provider tabs which have their own buttons) */}
          {activeTab !== "security" && activeTab !== "gallery" && activeTab !== "provider" && (
            <div className="sticky bottom-4 mt-6 z-10">
              <Card className={`shadow-lg ${Object.keys(formErrors).length > 0 && touched.size > 0 ? "border-destructive/40" : "border-primary/20"}`}>
                <CardContent className="py-3 flex items-center justify-between gap-4">
                  {Object.keys(formErrors).length > 0 && touched.size > 0 ? (
                    <p className="text-sm text-destructive hidden sm:flex items-center gap-1.5">
                      <CheckCircle2 className="h-4 w-4 shrink-0" />
                      {Object.keys(formErrors).length === 1
                        ? "1 field needs attention before saving."
                        : `${Object.keys(formErrors).length} fields need attention before saving.`}
                    </p>
                  ) : (
