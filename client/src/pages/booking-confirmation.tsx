...
export default function BookingConfirmation() {
  ...
  const priceLines: PriceLine[] = useMemo(() => {
    ...
    if (stored?.lines && stored.lines.length > 0) {
      // Show ALL stored lines — zero-amount lines are rendered as "Included" by the
      // line renderer so the patient can see every component of the price (no hidden breakups).
      // We only drop truly structural placeholders: lines with no label.
      let lines = stored.lines.filter(l => l.label?.trim());
      // Correct the platform fee line: stored snapshot from computeFinalPrice() uses
      // base.platformFee (pre-rule). The engine stores the overridden value in
      // appointments.platform_fee_amount. Update any 0-amount line from the column.
      lines = lines.map(l =>
        /platform\s*fee/i.test(l.label) && l.amount === 0 && platformFee > 0
          ? { ...l, amount: platformFee }
          : l,
      );
      // Re-inject platform fee from the column if there's no line at all
