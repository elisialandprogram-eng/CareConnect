...
const CATEGORY_ICON: Record<string, any> = {
  ...
  dental:               Sparkles,
  alternative_medicine: Users,
  nursing:              UserRound,
};

const CATEGORY_COLOR: Record<string, string> = {
  physician:            "from-blue-500/10 to-blue-500/0 text-blue-700 dark:text-blue-300 border-blue-500/20",
  mental_health:        "from-violet-500/10 to-violet-500/0 text-violet-700 dark:text-violet-300 border-violet-500/20",
  nutrition:            "from-lime-500/10 to-lime-500/0 text-lime-700 dark:text-lime-300 border-lime-500/20",
  rehabilitation:       "from-emerald-500/10 to-emerald-500/0 text-emerald-700 dark:text-emerald-300 border-emerald-500/20",
  dental:               "from-cyan-500/10 to-cyan-500/0 text-cyan-700 dark:text-cyan-300 border-cyan-500/20",
  alternative_medicine: "from-teal-500/10 to-teal-500/0 text-teal-700 dark:text-teal-300 border-teal-500/20",
  nursing:              "from-rose-500/10 to-rose-500/0 text-rose-700 dark:text-rose-300 border-rose-500/20",
};
                ...
                return (
                  <div key={cat.id} data-testid={`section-category-${cat.slug}`}>
                    ...
                    {cat.subServices.length === 0 ? (
                      <div className="flex items-center gap-2.5 py-3 px-4 rounded-lg bg-muted/40 border border-border/50" data-testid={`empty-subs-${cat.slug}`}>
                                ...
                                <div>
                                  ...
                                  >
                                    {s.startingPrice != null
                                      ? formatInCurrency(s.startingPrice, s.startingPriceCurrency ?? "USD")
