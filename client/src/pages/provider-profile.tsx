                    ...
                    <Card data-testid="card-profile-packages">
                      ...
                      <CardContent className="space-y-4">
                        {providerPackages.map((pkg) => {
                          // PRICE-DRIFT-FIX: pkg.price is stored in USD; service prices are now in native currency.
                          // Convert pkg.price USD→native before display and savings calculation.
                          const nativeCur = providerNativeCur as SupportedCurrency;
                          const pkgPriceNative = nativeCur === "USD"
                            ? Number(pkg.price)
                            : convertBetweenCurrencies(Number(pkg.price), "USD", nativeCur);
                          // Sum service prices in native currency (each may carry its own currency tag).
                          const fullPriceNative = (pkg.services as any[]).reduce((sum, s) => {
                            const n = Number(s.price ?? 0);
                            const sc = (s.currency ?? providerNativeCur) as SupportedCurrency;
                            const inNative = sc === nativeCur ? n : convertBetweenCurrencies(n, sc, nativeCur);
                            return sum + inNative;
                          }, 0);
                          const savings = Math.max(0, fullPriceNative - pkgPriceNative);
                          return (
                            <div
                              key={pkg.id}
                              ...
                              <div className="flex items-start justify-between gap-4 mb-2">
                                ...
                                <div className="text-right shrink-0">
                                  <p className="text-xl font-bold text-primary" data-testid={`text-profile-package-price-${pkg.id}`}>
                                    {formatInCurrency(pkgPriceNative, providerNativeCur)}
                                  </p>
                                  {savings > 0 && (
                                    <p className="text-xs text-muted-foreground line-through">
                                      {formatInCurrency(fullPriceNative, providerNativeCur)}
                                    </p>
                                  )}
                                </div>
                              </div>
