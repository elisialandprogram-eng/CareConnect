...
function getTimezoneOptions() {
  let zones: string[] = [];
  try { const s = (Intl as any).supportedValuesOf?.("timeZone") as string[] | undefined; if (s?.length) zones = s; } catch { /**/ }
  if (!zones.length) zones = COMMON_TIMEZONES;
  return zones
    .map((z) => { const off = tzOffsetLabel(z); return { value: z, label: off ? `(${off}) ${z.replace(/_/g, " ")}` : z.replace(/_/g, " ") }; })
...
export default function Settings() {
  ...
  usePageTitle(t("settings.meta_title", "Settings"));

  const [pushCap, setPushCap] = useState<{ supported: boolean; configured: boolean }>({ supported: false, configured: false });
  const [pushSubscribed, setPushSubscribed] = useState(false);

  const { data: prefs } = useQuery<any>({ queryKey: QK.notificationPreferences(), enabled: isAuthenticated });
  const { data: caps } = useQuery<any>({ queryKey: QK.commsCapabilities() });
  ...
  const updateCurrency = useMutation({
    mutationFn: async (preferredCurrency: string) =>
      apiRequest("PATCH", "/api/auth/profile", { preferredCurrency }),
    onSuccess: async () => {
      await refreshUser();
  ...
  const togglePush = async (on: boolean) => {
    try {
      if (on) {
        ...
        toast({ title: t("settings_page.push_enabled") });
      } else {
  ...
  const changePasswordMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast({
        title: t("settings_page.password_changed"),
        description: t("settings_page.password_changed_desc"),
      });
    },
    onError: (error: Error) => {
      showErrorModal({
  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      <main className="flex-1 container mx-auto px-4 py-8 max-w-2xl">
        <PageBreadcrumbs
          items={[{ label: "Home", href: "/" }, { label: t("settings_page.title", "Settings") }]}
          fallback="/"
        />
        ...
        <div className="space-y-6">
          ...
          {/* Currency preference — hidden for admins (admin view is always USD) */}
          {user && !["admin", "global_admin", "country_admin"].includes((user as any).role) && (
            <Card>
              <CardHeader>
                <div className="flex items-center gap-2">
                  <Banknote className="h-5 w-5 text-primary" />
                  <CardTitle className="text-lg">{t("settings_page.display_currency_title")}</CardTitle>
                </div>
                <CardDescription>
                  {t("settings_page.display_currency_desc")}
                </CardDescription>
              </CardHeader>
              <CardContent>
                <p className="font-medium mb-2">{t("settings_page.preferred_currency_label")}</p>
                <select
                  data-testid="select-preferred-currency"
                  className="w-full border border-input rounded-md h-10 px-3 bg-background"
                  value={(user as any).preferredCurrency || "USD"}
                ...
                {updateCurrency.isPending && (
                  <p className="text-xs text-muted-foreground mt-2">{t("settings_page.saving")}</p>
                )}
              </CardContent>
            </Card>
          )}
