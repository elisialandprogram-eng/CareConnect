...
function BrowseList() {
  ...
  const { format: fmtMoney } = useCurrency();
  const list = useQuery<GroupSession[]>({ queryKey: QK.groupSessions() });
  const book = useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: QK.groupSessions() });
      queryClient.invalidateQueries({ queryKey: QK.myGroupSessions() });
            ...
            <CardContent className="space-y-3">
              ...
              <div className="flex items-center justify-between">
                <div className="text-lg font-semibold">
                  {formatInCurrency(Number(s.pricePerUser), getCurrencyConfigForCountry(s.countryCode).code)}
                </div>
                <Button
...
function MyBookingsList() {
  ...
  const { toast } = useToast();
  const { format: fmtMoney } = useCurrency();
  const list = useQuery<MyBooking[]>({ queryKey: QK.myGroupSessions() });
