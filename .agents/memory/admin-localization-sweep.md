...
**How to apply:** When adding admin-visible copy, add the key to the overlay for all supported locales, use the existing admin namespace, and map known notification/event types to translated titles while preserving unknown server content as a fallback.